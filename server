...
app.get('/student/:id', async(req,res)=>{
  ...
  res.send(layout('Technician Home', `<div class="card"><span class="pill">Class ${esc(s.code)}</span><div class="big">Welcome, ${esc(s.name)}</div><p>${esc(s.course)} · ${esc(s.dealer)}</p></div><div class="grid"><div class="card"><div class="big">Module Quiz</div><p>Knowledge test. Your instructor can watch your progress while you work.</p>${done.has('Module Quiz')?`<div class="success">Completed</div><div style="margin-top:10px"><a class="btn light" href="/student/${s.id}/quiz-review?token=${encodeURIComponent(s.join_token)}">Review Missed Questions</a></div>`:`<a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Start Quiz</a>`}</div><div class="card"><div class="big">QR Scavenger Hunt</div><p>Scan the QR code posted at each training station. This phone will remember who you are.</p>${done.has('Scavenger Hunt')?'<div class="success">Completed</div>':`<a class="btn" href="/student/${s.id}/hunt?token=${encodeURIComponent(s.join_token)}">View Hunt Progress</a>`}</div><div class="card"><div class="big">Failure Simulation</div><p>Work through a crank/no-start diagnostic scenario.</p>${done.has('Failure Simulation')?'<div class="success">Completed</div>':`<a class="btn" href="/student/${s.id}/scenario?token=${encodeURIComponent(s.join_token)}">Start Simulation</a>`}</div><div class="card"><div class="big">Training Feedback</div><p>Tell us what helped and what should be improved.</p>${feedback?'<div class="success">Feedback Submitted — Thank You</div>':allCore?`<a class="btn" href="/student/${s.id}/feedback?token=${encodeURIComponent(s.join_token)}">Give Training Feedback</a>`:'<div class="muted">Available after the training activities are finished.</div>'}</div></div>`));
});
...
app.post('/student/:id/quiz', async(req,res)=>{
  ...
  const q=await pool.query('SELECT * FROM quiz_questions WHERE id=ANY($1::int[])',[ids]);
  const byId=new Map(q.rows.map(x=>[Number(x.id),x]));
  let correct=0; const review=[];
  for(const id of ids){
    const x=byId.get(Number(id)); if(!x) continue;
    const raw=req.body['q_'+x.id];
    const selectedIndex=(raw===undefined||raw==='')?null:Number(raw);
    const choices=Array.isArray(x.choices)?x.choices:[];
    const selectedAnswer=selectedIndex===null?'No answer':(choices[selectedIndex]??'No answer');
    const correctAnswer=choices[x.answer_index]??'Correct answer unavailable';
    const ok=selectedIndex!==null && selectedIndex===x.answer_index;
    if(ok) correct++;
    review.push({
      question_id:Number(x.id),
      question:x.question,
      topic:x.topic||'',
      choices,
      ...
      correct_index:Number(x.answer_index),
      correct:correctAnswer,
      correct_answer:correctAnswer,
      explanation:x.explanation||`The correct answer is ${correctAnswer}. Review this topic with your instructor.`,
      is_correct:ok
    });
  }
  const total=review.length;
  const missed=review.filter(x=>!x.is_correct);
  const score=total?Math.round(correct/total*100):0;
  // Keep one current Module Quiz result per student so review always opens the exact latest submission.
  await pool.query("DELETE FROM results WHERE student_id=$1 AND activity='Module Quiz'",[s.id]);
  await pool.query("INSERT INTO results(student_id,activity,score,details) VALUES($1,'Module Quiz',$2,$3)",[s.id,score,JSON.stringify({version:2,correct,total,missed,review})]);
  ...
  const reviewHtml=`<div class="card"><div class="big">Quiz Review — Questions You Missed</div><p class="muted">For every missed question, you can see what you chose, what the correct answer was, and why.</p>${missed.length?missed.map((m,i)=>`<div class="q feedback-miss"><b>Missed Question ${i+1}${m.topic?' · '+esc(m.topic):''}</b><p><b>Question:</b> ${esc(m.question)}</p><p><b>Your Answer:</b> ${esc(m.student_answer)}</p><p><b>Correct Answer:</b> ${esc(m.correct_answer)}</p><p><b>Explanation:</b> ${esc(m.explanation)}</p></div>`).join(''):'<div class="success">You did not miss any questions.</div>'}</div>`;
  res.send(layout('Quiz Complete', `<div class="card center"><div class="big">Quiz Complete</div><div class="code">${score}%</div><p>${correct} of ${total} correct</p><div class="toolbar" style="justify-content:center"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a><a class="btn" href="/student/${s.id}/quiz-review?token=${encodeURIComponent(s.join_token)}">Review Missed Questions</a></div></div>${reviewHtml}`));
});
...
async function buildStudentQuizReview(studentId){
  const rr=await pool.query("SELECT details FROM results WHERE student_id=$1 AND activity='Module Quiz' ORDER BY completed_at DESC LIMIT 1",[studentId]);
  if(!rr.rowCount) return {found:false,html:'<div class="alert">No completed quiz was found for this student.</div>'};
  const details=rr.rows[0].details||{};
  let review=Array.isArray(details.review)?details.review:[];
  let missed=review.length?review.filter(x=>!x.is_correct):(Array.isArray(details.missed)?details.missed:[]);
  const cards=[];
  for(let i=0;i<missed.length;i++){
    let m=missed[i]; if(typeof m==='string') m={question:m};
    const question=m.question||'Question text not recorded';
    let selected=m.student_answer||m.selected||'No answer recorded';
    let correct=m.correct_answer||m.correct||'';
    let explanation=m.explanation||'';
    let topic=m.topic||'';
    if((!correct || !explanation) && question){
      const qq=await pool.query('SELECT choices,answer_index,explanation,topic FROM quiz_questions WHERE question=$1 ORDER BY id DESC LIMIT 1',[question]);
      if(qq.rowCount){
        const x=qq.rows[0], choices=Array.isArray(x.choices)?x.choices:[];
        if(!correct) correct=choices[x.answer_index]||'';
        if(!explanation) explanation=x.explanation|| (correct?`The correct answer is ${correct}. Review this topic with your instructor.`:'Review this topic with your instructor.');
        if(!topic) topic=x.topic||'';
      }
    }
    cards.push(`<div class="q feedback-miss"><b>Missed Question ${i+1}${topic?' · '+esc(topic):''}</b><p><b>Question:</b> ${esc(question)}</p><p><b>Your Answer:</b> ${esc(selected)}</p><p><b>Correct Answer:</b> ${esc(correct||'Correct answer was not stored in this older attempt. Retake the quiz once to create a complete review.')}</p><p><b>Explanation:</b> ${esc(explanation||'Explanation was not stored in this older attempt.')}</p></div>`);
  }
  return {found:true,html:cards.length?cards.join(''):'<div class="success">You did not miss any questions.</div>'};
}

app.get('/student/:id/quiz-review', async(req,res)=>{
  const s=await studentContext(req.params.id,req.query.token); if(!s) return res.status(403).send('Invalid student link.');
  const review=await buildStudentQuizReview(s.id);
  res.send(layout('Quiz Review', `<div class="toolbar no-print"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">← Back to Training</a></div><div class="card"><div class="eyebrow">Student Quiz Feedback</div><div class="big">What You Missed</div><p class="muted">For each missed question, your answer, the correct answer, and the explanation are shown below.</p>${review.html}</div>`));
});
...
app.get('/instructor/student/:id',auth,async(req,res)=>{
  ...
  const reviewData=await buildStudentQuizReview(s.id); const quizReview=reviewData.html;
...
app.get('/instructor/student/:id/report',auth,async(req,res)=>{
  ...
  const reportReview=await buildStudentQuizReview(s.id); const missedRows=reportReview.html;
