...
const DIAGNOSTIC_SIMULATIONS = {
  '5100-crank-no-start': {
    ...
    correctDiagnosis:'Failed camshaft position sensor',
    correctRepair:'Replace camshaft position sensor and verify synchronization',
    actions:[
      ...
      {key:'live_data',category:'Live Data',label:'View Cranking Live Data',points:15,result:'Engine RPM: 215 RPM | Rail pressure: 4.2 MPa | Cam synchronization: NO | Battery voltage: 10.9 V | Engine state: 33'},
      {key:'rail_pressure',category:'Pressure Tests',label:'Check Rail Pressure While Cranking',points:0,result:'Measured rail pressure while cranking: 4.2 MPa. Compare this value with service information before condemning a component.'},
      {key:'cam_circuit',category:'Component Tests',label:'Check Cam Sensor Circuit',points:20,result:'Cam sensor connector and harness are intact. Power and ground are present. Signal must be tested while cranking.'},
      {key:'cam_meter',category:'Multimeter',label:'Meter Test — Cam Sensor Power / Ground / Signal',points:15,result:'Supply: 4.98 V (spec 5.0 V) | Ground voltage drop: 0.03 V | Signal: no switching signal while cranking.'},
      {key:'wiring',category:'Wiring Diagram',label:'Open Cam Sensor Wiring Diagram',points:0,result:'Diagram shows a 5 V reference, sensor ground, and signal circuit between the camshaft position sensor and ECU.'},
      {key:'service_info',category:'Service Information',label:'Review Diagnostic Procedure',points:0,result:'Service procedure: verify cranking RPM, synchronization status, sensor power/ground, then verify sensor signal before component replacement.'},
      {key:'fuel_filter',category:'Component Tests',label:'Inspect Fuel Filter / Supply',points:0,result:'Fuel supply is present and the filter is not restricted. No evidence here proves a fuel-filter failure.'},
      {key:'injector_resistance',category:'Component Tests',label:'Check Injector Resistance',points:0,result:'Injector electrical checks are consistent across all cylinders. This test does not explain the missing cam synchronization.'},
      {key:'replace_ecu',category:'Replace Part',label:'Replace ECU',penalty:15,result:'PARTS CANNON PENALTY: ECU replaced without proof of ECU failure. Concern remains.'},
      {key:'replace_hp_pump',category:'Replace Part',label:'Replace High-Pressure Pump',penalty:15,result:'PARTS CANNON PENALTY: high-pressure pump replaced without proving it failed. Concern remains.'},
      {key:'replace_injectors',category:'Replace Part',label:'Replace Injectors',penalty:15,result:'PARTS CANNON PENALTY: injectors replaced without proof of injector failure. Concern remains.'}
    ]
  }
};
...
app.get('/student/:id', async(req,res)=>{
  ...
  res.send(layout('Technician Home', `<div style="background:linear-gradient(135deg,#171717,#3a080b);color:white;border-radius:18px;padding:24px;margin-bottom:18px;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#f0b8bb">Technician Training Portal · v5.1</div><div class="big" style="font-size:30px">Welcome, ${esc(s.name)}</div><p style="margin-bottom:0">${esc(s.course)} · ${esc(s.dealer)} · Class ${esc(s.code)}</p></div>${certificateCard}<div class="grid"><div class="card"><div class="big">Module Quiz</div><p>Knowledge test. Your instructor can watch your progress while you work.</p>${done.has('Module Quiz')?`<div class="success">Latest Score · ${quizScore}%</div><div class="toolbar" style="margin-top:10px"><a class="btn light" href="/student/${s.id}/quiz-review?token=${encodeURIComponent(s.join_token)}">Review Latest Answers</a><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a></div>`:`<a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Start Quiz</a>`}</div><div class="card"><div class="big">QR Scavenger Hunt</div><p>Scan the QR code posted at each training station.</p>${done.has('Scavenger Hunt')?'<div class="success">Completed</div>':`<a class="btn" href="/student/${s.id}/hunt?token=${encodeURIComponent(s.join_token)}">View Hunt Progress</a>`}</div><div class="card"><div class="big">Diagnostic Failure Simulation</div><p>Enter the virtual shop bay and diagnose a crank/no-start failure step by step.</p>${done.has('Failure Simulation')?`<div class="success">Latest simulation completed</div><a class="btn" style="margin-top:10px" href="/student/${s.id}/scenario?token=${encodeURIComponent(s.join_token)}">Review / Retake Simulation</a>`:`<a class="btn" href="/student/${s.id}/scenario?token=${encodeURIComponent(s.join_token)}">Enter Diagnostic Bay</a>`}</div><div class="card"><div class="big">Training Feedback</div><p>Tell us what helped and what should be improved.</p>${feedback?'<div class="success">Feedback Submitted — Thank You</div>':allCore?`<a class="btn" href="/student/${s.id}/feedback?token=${encodeURIComponent(s.join_token)}">Give Training Feedback</a>`:'<div class="muted">Available after the training activities are finished.</div>'}</div></div>`));
});
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.1 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.1 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/student/:id/scenario', async(req,res)=>{
  ...
  const sim=simByKey('5100-crank-no-start');
...
app.post('/student/:id/scenario/start', async(req,res)=>{
  ...
  const sim=simByKey('5100-crank-no-start');
  const a=(await pool.query(`INSERT INTO simulation_attempts(student_id,class_id,simulation_key,title,work_order,status,started_at) VALUES($1,$2,$3,$4,$5,'in_progress',now()) RETURNING *`,[s.id,s.class_id,sim.key,sim.title,sim.workOrder])).rows[0];
...
app.get('/student/:id/scenario/:attemptId', async(req,res)=>{
  ...
  const sim=simByKey(a.simulation_key); const actions=(await pool.query('SELECT * FROM simulation_actions WHERE attempt_id=$1 ORDER BY sequence',[a.id])).rows;
  ...
  const finalForm=`<div class="card sim-final"><div class="big">Final Diagnosis</div><p>When you believe you have enough evidence, choose the root cause and repair. You can continue testing before submitting.</p><form method="post" action="/student/${s.id}/scenario/${a.id}/final?token=${encodeURIComponent(s.join_token)}"><label>Root Cause<select name="diagnosis" required><option value="">Choose the most likely root cause</option><option>Failed camshaft position sensor</option><option>Failed engine ECU</option><option>Restricted fuel filter</option><option>Failed high-pressure fuel pump</option><option>All injectors failed</option></select></label><label>Repair / Verification<select name="repair" required><option value="">Choose the repair and verification</option><option>Replace camshaft position sensor and verify synchronization</option><option>Replace ECU and clear codes</option><option>Replace high-pressure pump and release tractor</option><option>Replace fuel filters only</option><option>Replace injectors and retest</option></select></label><button>Submit Final Diagnosis</button></form></div>`;
...
app.post('/student/:id/scenario/:attemptId/action', async(req,res)=>{
  ...
  const sim=simByKey(a.simulation_key), action=sim.actions.find(x=>x.key===req.body.action_key); if(!action) return res.status(400).send('Unknown diagnostic action');
  ...
  await pool.query(`INSERT INTO simulation_actions(attempt_id,action_key,category,label,result_text,points,penalty,sequence) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(attempt_id,action_key) DO NOTHING`,[a.id,action.key,action.category,action.label,action.result,action.points||0,action.penalty||0,seq]);
...
app.post('/student/:id/scenario/:attemptId/final', async(req,res)=>{
  const s=await studentContext(req.params.id,req.query.token); if(!s) return res.status(403).send('Invalid session');
  const a=await getSimAttemptForStudent(s.id,Number(req.params.attemptId)); if(!a||a.status!=='in_progress') return res.status(400).send('Simulation is not active');
  const sim=simByKey(a.simulation_key); const diagnosis=(req.body.diagnosis||'').trim(), repair=(req.body.repair||'').trim();
  const diagnosisCorrect=diagnosis===sim.correctDiagnosis, repairCorrect=diagnosisCorrect && repair===sim.correctRepair;
...
app.get('/student/:id/scenario/:attemptId/review', async(req,res)=>{
  const s=await studentContext(req.params.id,req.query.token); if(!s) return res.status(403).send('Invalid session');
  const a=await getSimAttemptForStudent(s.id,Number(req.params.attemptId)); if(!a) return res.status(404).send('Simulation attempt not found');
  const sim=simByKey(a.simulation_key), actions=(await pool.query('SELECT * FROM simulation_actions WHERE attempt_id=$1 ORDER BY sequence',[a.id])).rows;
  ...
  res.send(layout('Simulation Review', `<div class="card" style="border-top:6px solid var(--red)"><div class="eyebrow">Permanent Diagnostic Simulation Attempt #${a.id}</div><div class="big">${esc(sim.title)}</div><div class="grid" style="margin-top:12px"><div class="stat"><span>SCORE</span><b>${a.score}%</b></div><div class="stat"><span>PROCESS POINTS</span><b>${a.positive_points}</b></div><div class="stat"><span>PENALTIES</span><b>-${a.penalty_points}</b></div><div class="stat"><span>TIME</span><b>${elapsedFmt}</b></div></div></div><div class="card"><div class="big">Final Diagnosis</div><p><b>Technician Diagnosis:</b> ${esc(a.final_diagnosis||'Not submitted')} ${a.diagnosis_correct?'<span class="pill open">CORRECT</span>':'<span class="pill">INCORRECT</span>'}</p><p><b>Correct Diagnosis:</b> ${esc(sim.correctDiagnosis)}</p><p><b>Technician Repair:</b> ${esc(a.final_repair||'Not submitted')}</p><p><b>Correct Repair:</b> ${esc(sim.correctRepair)}</p></div><div class="card"><div class="big">Diagnostic Process Review</div>${rows||'<p>No diagnostic actions recorded.</p>'}</div><div class="card"><div class="big">Scoring Breakdown</div><table><tr><th>Area</th><th>Points</th></tr><tr><td>Verify customer complaint</td><td>10</td></tr><tr><td>Check fault codes</td><td>10</td></tr><tr><td>Review live data</td><td>15</td></tr><tr><td>Select correct circuit test</td><td>20</td></tr><tr><td>Use multimeter correctly</td><td>15</td></tr><tr><td>Identify root cause</td><td>20</td></tr><tr><td>Verify repair</td><td>10</td></tr><tr><th>Total Possible</th><th>100</th></tr></table>${a.penalty_points?`<div class="parts-penalty" style="margin-top:12px">Parts Cannon Penalty Applied: -${a.penalty_points} points</div>`:''}</div><div class="toolbar"><a class="btn" href="/student/${s.id}/scenario?token=${encodeURIComponent(s.join_token)}">Run Simulation Again</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`,simulationCss()));
});
...
app.get('/instructor/simulations',auth,async(req,res)=>{
  ...
  const sims=Object.values(DIAGNOSTIC_SIMULATIONS).map(sim=>`<div class="card" style="border-top:5px solid var(--red)"><div class="eyebrow">AVAILABLE SIMULATION</div><div class="big">${esc(sim.title)}</div><p><b>Model:</b> ${esc(sim.model)} &nbsp; · &nbsp; <b>Work Order:</b> ${esc(sim.workOrder)} &nbsp; · &nbsp; <b>Hours:</b> ${esc(sim.hours)}</p><div class="alert"><b>Customer Complaint:</b> ${esc(sim.complaint)}</div><p><b>Instructor use:</b> This simulation is automatically available to students from their Training Home page. Students enter the virtual shop bay, choose diagnostic actions, submit a final diagnosis and repair, and the attempt is saved here for review.</p><div class="toolbar"><a class="btn" href="/instructor/simulations/${encodeURIComponent(sim.key)}/preview?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Preview Simulation</a></div></div>`).join('');
  res.send(layout('Diagnostic Simulations', `<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Command Center</a></div><div class="card" style="background:linear-gradient(135deg,#171717,#4b090d);color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">Diagnostic Failure Simulation System · v5.1</div><div class="big" style="font-size:30px">Simulation Library</div><p style="margin-bottom:0">Available simulations appear here even before any technician has started one.</p></div>${sims}<div class="card"><div class="big">Technician Attempts</div><p>Permanent record of each technician's diagnostic path, test sequence, parts-cannon penalties, final diagnosis, repair selection, score, and elapsed time.</p><div style="overflow:auto"><table class="sim-history"><thead><tr><th>Technician</th><th>Dealer</th><th>Simulation</th><th>Score</th><th>Penalty</th><th>Date</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="7">No technician has run a simulation yet. The simulation above is ready for students now.</td></tr>'}</tbody></table></div></div>`,simulationCss()));
});
...
app.get('/instructor/simulation/:attemptId',auth,async(req,res)=>{
  ...
  res.send(layout('Simulation Attempt', `<div class="toolbar"><a class="btn light" href="/instructor/simulations?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Simulations</a><a class="btn light" href="/instructor/student/${a.student_id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Student Record</a></div><div class="card"><div class="eyebrow">Permanent Simulation Attempt #${a.id}</div><div class="big">${esc(a.name)} · ${esc(sim.title)}</div><p>${esc(a.dealer)} · Class ${esc(a.code)}</p><div class="grid"><div class="stat"><span>SCORE</span><b>${a.score}%</b></div><div class="stat"><span>EARNED</span><b>${a.positive_points}</b></div><div class="stat"><span>PENALTY</span><b>-${a.penalty_points}</b></div><div class="stat"><span>STATUS</span><b>${esc(a.status)}</b></div></div></div><div class="card"><div class="big">Final Decision</div><p><b>Diagnosis:</b> ${esc(a.final_diagnosis||'Not submitted')} ${a.diagnosis_correct?'✓':'✗'}</p><p><b>Correct Diagnosis:</b> ${esc(sim.correctDiagnosis)}</p><p><b>Repair:</b> ${esc(a.final_repair||'Not submitted')}</p><p><b>Correct Repair:</b> ${esc(sim.correctRepair)}</p></div><div class="card"><div class="big">Diagnostic Path</div>${rows||'<p>No actions recorded.</p>'}</div>`,simulationCss()));
});
