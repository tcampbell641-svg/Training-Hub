...
app.post('/student/:id/quiz', async(req,res)=>{
  ...
  const reviewHtml=`<div class="card"><div class="big">Quiz Review</div><p class="muted">Review every question you missed. The correct answer and explanation are shown below.</p>${missed.length?missed.map((m,i)=>`<div class="q feedback-miss"><b>Missed Question ${i+1}${m.topic?' · '+esc(m.topic):''}</b><p><b>Question:</b> ${esc(m.question)}</p><p><b>Your Answer:</b> ${esc(m.selected)}</p><p><b>Correct Answer:</b> ${esc(m.correct)}</p><p><b>Explanation:</b> ${esc(m.explanation)}</p></div>`).join(''):'<div class="success">You did not miss any questions.</div>'}</div>`;
...
async function buildStudentQuizReview(studentId){
  ...
  for(let i=0;i<missed.length;i++){
    let m=missed[i];
    if(typeof m==='string') m={question:m};
    let question=m.question||'';
    let selected=m.selected||m.student_answer||'Not recorded';
    let correct=m.correct||m.correct_answer||'';
    ...
    if((!correct || !explanation) && question){
      ...
      if(qq.rowCount){
        const q=qq.rows[0];
        if(!correct && Array.isArray(q.choices)) correct=q.choices[q.answer_index]||'';
        if(!explanation) explanation=q.explanation||'Review this topic with your instructor.';
        if(!topic) topic=q.topic||'';
      }
    }
    cards.push(`<div class="q feedback-miss"><b>Missed Question ${i+1}${topic?' · '+esc(topic):''}</b><p><b>Question:</b> ${esc(question||'Question text not recorded')}</p><p><b>Your Answer:</b> ${esc(selected)}</p><p><b>Correct Answer:</b> ${esc(correct||'Correct answer was not stored in this older result. Retake the quiz to create a full review.')}</p><p><b>Explanation:</b> ${esc(explanation||'Explanation was not stored in this older result.')}</p></div>`);
  }
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  const s=await studentFrom(req); if(!s) return res.status(403).send('Invalid student link.');
