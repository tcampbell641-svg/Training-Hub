...
{name:'Recorder',duty:'Enter the team verified answer or measurement into the Training Hub.'},
...
app.get('/student/:id/hunt', async(req,res)=>{
  ...
  const tractorCount=Math.min(5,Math.max(1,Number(s.hunt_tractor_count)||3)); const team=huntTeamInfo(rosterIndex,tractorCount); const startingRole=huntRoleForStation(team.memberSlot,1);
  ...
  const tractorName=n=>models.get(n)||`Tractor ${n}`; const rotation=team.rotation.map(n=>tractorName(n)).join(' → ');
  ...
  res.send(layout('Scavenger Hunt', `<div class="card"><div class="eyebrow">Hands-On Team Activity</div><div class="big">Tractor Technician Scavenger Hunt</div><div class="grid"><div class="stat"><span>Your Team</span><b>${team.name}</b></div><div class="stat"><span>Start At</span><b>${esc(tractorName(team.rotation[0]))}</b></div><div class="stat"><span>Your First Role</span><b>${startingRole.name}</b></div><div class="stat"><span>Progress</span><b>${completed.size} / ${q.rowCount}</b></div></div><div class="alert"><b>Rotation:</b> ${esc(rotation)}<br>Your role changes automatically at every station.</div><p><b>Roles:</b> Navigator · Locator · Tool Operator · Recorder · Verifier</p></div>${groups}<div class="card"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`));
});
...
app.get('/hunt-station/:cid/:sid', async(req,res)=>{
  ...
  const done=(await pool.query('SELECT * FROM hunt_progress WHERE student_id=$1 AND station_id=$2',[student.id,station.id])).rows[0]; const roster=(await pool.query('SELECT id FROM students WHERE class_id=$1 ORDER BY joined_at,id',[station.class_id])).rows; const rosterIndex=Math.max(0,roster.findIndex(x=>Number(x.id)===Number(student.id))); const team=huntTeamInfo(rosterIndex,station.hunt_tractor_count);
  const stationOrder=(await pool.query('SELECT id FROM hunt_stations WHERE class_id=$1 ORDER BY COALESCE(tractor_no,99),id',[station.class_id])).rows; const stationNumber=Math.max(1,stationOrder.findIndex(x=>Number(x.id)===Number(station.id))+1); const role=huntRoleForStation(team.memberSlot,stationNumber),nextRole=huntRoleForStation(team.memberSlot,stationNumber+1);
  res.send(layout(station.station_name, `<div class="card"><span class="pill">${team.name}</span><span class="pill hunt" style="margin-left:6px">Station ${stationNumber}</span><div class="big" style="margin-top:12px">${esc(station.item_label||station.station_name)}</div><p class="muted">${esc(station.tractor_model||'')} ${station.tractor_no?`· Tractor ${station.tractor_no}`:''}</p><div class="alert"><div class="eyebrow">YOUR ROLE AT THIS STATION</div><div class="big">${esc(role.name)}</div><p>${esc(role.duty)}</p></div><h3>Team Task</h3><p>${esc(station.task)}</p>${done?`<div class="success">Completed. Your answer: <b>${esc(done.answer)}</b><br>Next role: <b>${esc(nextRole.name)}</b></div><a class="btn light" href="/student/${student.id}/hunt?token=${encodeURIComponent(station.join_token)}">View Hunt Progress</a>`:`<form method="post" action="/hunt-station/${station.class_id}/${station.id}"><input type="hidden" name="student_id" value="${student.id}"><input type="hidden" name="token" value="${esc(station.join_token)}"><label>Your Answer / Measurement<input name="answer" required autofocus></label><button>Submit Station</button></form><p class="small muted">Next station role: <b>${esc(nextRole.name)}</b></p>`}</div>`));
});
  ...
  const sims=Object.values(DIAGNOSTIC_SIMULATIONS).map(sim=>{
    ...
    return `<div class="card" style="border-top:5px solid var(--red)">
      ...
      <form method="post" action="/instructor/simulations/${encodeURIComponent(sim.key)}/configure?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="margin-top:14px">
        <label>Class<select name="class_id" required><option value="">Choose class</option>${classes.map(c=>`<option value="${c.id}" ${Number(selectedClass)===Number(c.id)?'selected':''}>${esc(c.code)} · ${c.student_count} technician(s) · ${c.active?'Active':'Closed'}</option>`).join('')}</select></label>
      ...
      ${selectedClass&&selectedRoot&&selectedTractor?`<div class="success" style="margin-top:10px"><b>Instructor Only:</b> Class ${esc(selectedClassRow?.code||selectedClass)} · Tractor: ${esc(selectedTractor)} · Root Cause: ${esc(rootCauseByKey(sim,selectedRoot)?.diagnosis||'')}</div>`:''}
...
app.get('/instructor/hunt/:id',auth,async(req,res)=>{
 const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c) return res.status(404).send('Class not found'); const count=Math.min(5,Math.max(1,Number(c.hunt_tractor_count)||3));
 ...
 const modelOptions=HUNT_MODEL_SUGGESTIONS.map(x=>`<option value="${esc(x)}">`).join(''); const itemChecks=HUNT_ITEM_LIBRARY.map(x=>`<label style="font-weight:500;margin:6px 0"><input style="width:auto;margin-right:8px" type="checkbox" name="items" value="${esc(x.id)}">${esc(x.label)}</label>`).join('');
 const tractorCards=Array.from({length:count},(_,i)=>i+1).map(n=>{const model=models.get(n)||''; const items=hs.rows.filter(x=>Number(x.tractor_no)===n);return `<div class="card"><div class="eyebrow">TRACTOR ${n}</div><div class="big">${esc(model||'Model Not Set')}</div><form method="post" action="/instructor/hunt/${c.id}/tractor/${n}/model"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Tractor Model<input list="hunt-models" name="model" value="${esc(model)}" placeholder="Example: Fifty One Hundred Refresh" required></label><button>Save Model</button></form><hr><form method="post" action="/instructor/hunt/${c.id}/tractor/${n}/items"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><div class="small muted">Choose the items students will hunt on this tractor.</div>${itemChecks}<button>Add Selected Hunt Items</button></form><p><b>${items.length}</b> hunt item(s) currently assigned.</p></div>`}).join('');
