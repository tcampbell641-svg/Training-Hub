...
:root{--red:#b5121b;--black:#171717;--line:#ddd;--soft:#f7f7f7}*{box-sizing:border-box}body{margin:0;font-family:Segoe UI,Arial,sans-serif;color:#1d1d1d;background:#fff}.top{background:var(--black);color:#fff;padding:16px 20px;border-bottom:5px solid var(--red)}.top b{font-size:20px}.wrap{max-width:1100px;margin:auto;padding:20px}.card{border:1px solid var(--line);border-radius:14px;padding:18px;margin:14px 0;background:#fff}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}.btn,button{display:inline-block;background:var(--red);color:#fff;border:0;border-radius:10px;padding:12px 16px;font-weight:700;text-decoration:none;cursor:pointer}.btn.alt{background:#333}.btn.light{background:#eee;color:#222}.big{font-size:24px;font-weight:800}.muted{color:#666}.code{font-size:44px;font-weight:900;letter-spacing:5px}.stat{background:var(--soft);padding:14px;border-radius:12px}.stat b{display:block;font-size:26px}input,select,textarea{width:100%;padding:12px;border:1px solid #bbb;border-radius:9px;font-size:16px;margin-top:5px}label{font-weight:650;display:block;margin:12px 0}.q{padding:14px;border:1px solid #ddd;border-radius:10px;margin:12px 0}.q label{font-weight:400;margin:8px 0}.q input[type=radio]{width:auto;margin-right:8px}table{width:100%;border-collapse:collapse}th,td{padding:10px;border-bottom:1px solid #ddd;text-align:left}.pass{color:#0b6d2f;font-weight:800}.review{color:#a14500;font-weight:800}.danger{color:#a00000}.qr{max-width:280px;width:100%;height:auto}.center{text-align:center}.steps{font-size:18px;line-height:1.6}.pill{display:inline-block;background:#eee;padding:6px 10px;border-radius:99px;font-size:13px;font-weight:700}.toolbar{display:flex;gap:8px;flex-wrap:wrap}.alert{padding:12px;border-radius:9px;background:#fff3cd;border:1px solid #ffe69c}.success{padding:12px;border-radius:9px;background:#e8f5ea;border:1px solid #b9dfc0}@media print{.no-print,.top{display:none!important}.wrap{max-width:none;padding:0}.card{border:0}.report{font-size:12pt}body{background:#fff}}
</style>${extra}</head><body><div class="top"><b>${esc(APP_NAME)}</b></div><div class="wrap">${body}</div></body></html>`; }
...
app.get('/instructor/dashboard',auth,async(req,res)=>{
  ...
  const rows=classes.rows.map(c=>`<tr><td><b>${esc(c.course)}</b><br><span class="muted">${esc(c.title)}</span></td><td>${esc(c.code)}</td><td>${c.students}</td><td>${c.active?'Open':'Closed'}</td><td><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Open</a></td></tr>`).join('');
  res.send(layout('Instructor Dashboard', `<div class="toolbar no-print"><a class="btn" href="/instructor/new?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Start New Class</a><a class="btn alt" href="/instructor/history?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Student Records</a></div><div class="card"><div class="big">Instructor Dashboard</div><p class="muted">Cloud mode: technicians can join from cellular or any internet connection.</p><div style="overflow:auto"><table><thead><tr><th>Class</th><th>Code</th><th>Students</th><th>Status</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="5">No classes yet.</td></tr>'}</tbody></table></div></div>`));
});
...
app.get('/instructor/class/:id',auth,async(req,res)=>{
  ...
  const origin=`${req.protocol}://${req.get('host')}`, joinUrl=`${origin}/c/${c.join_token}`; const qr=await QRCode.toDataURL(joinUrl,{width:320,margin:1});
  const rows=students.rows.map(s=>`<tr><td><b>${esc(s.name)}</b><br><span class="muted">${esc(s.dealer)}</span></td><td>${s.activities}</td><td>${s.avg_score}%</td><td><a class="btn light" href="/instructor/student/${s.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Open Record</a></td></tr>`).join('');
  res.send(layout('Class', `<div class="toolbar no-print"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Dashboard</a><a class="btn alt" href="/instructor/builder/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Build Training</a></div><div class="grid"><div class="card"><span class="pill">${c.active?'CLASS OPEN':'CLASS CLOSED'}</span><div class="big" style="margin-top:10px">${esc(c.course)}</div><p>${esc(c.title)} · ${esc(c.instructor)}</p><div class="code">${esc(c.code)}</div><p class="muted">Technicians can scan the QR code or enter this six-digit class code.</p><form class="no-print" method="post" action="/instructor/class/${c.id}/toggle"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="${c.active?'btn alt':'btn'}">${c.active?'Close Class':'Reopen Class'}</button></form></div><div class="card center"><img class="qr" src="${qr}" alt="Technician class QR code"><div><a href="${esc(joinUrl)}">${esc(joinUrl)}</a></div><p class="muted">This is an internet address — not your laptop's local IP.</p></div></div><div class="card"><div class="big">Live Student Results</div><meta http-equiv="refresh" content="10"><div style="overflow:auto"><table><thead><tr><th>Technician</th><th>Activities</th><th>Average</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="4">Waiting for technicians to join...</td></tr>'}</tbody></table></div><p class="muted">This page refreshes every 10 seconds while open.</p></div>`));
});
