...
const SIMULATION_TRACTORS = {
  '5100-crank-no-start':['5100'],
  '6000-no-injector-activation':['6065','6075'],
  '3100-hst-no-forward':['3100 HST'],
  'can-network-down':['5100','6065','6075','3100 HST','5155'],
  '6075-shuttle-reverse':['6075'],
  '5145-hitch-low':['5155']
};
function tractorChoicesForSim(sim){ return SIMULATION_TRACTORS[sim.key]||[sim.model.replace(/^Mahindra\s+/,'')]; }
...
function simulationDisplayTitle(sim,model){
  ...
  if(sim.key==='5100-crank-no-start') return `${m} — Cranks, No Start`;
  if(sim.key==='6000-no-injector-activation') return `${m} — Cranks, No Injector Activation`;
  if(sim.key==='3100-hst-no-forward') return `${m} — No Forward Travel`;
  if(sim.key==='can-network-down') return `${m} — CAN Network: Multiple Modules Not Communicating`;
  if(sim.key==='6075-shuttle-reverse') return `${m} — Delayed / No Reverse`;
  if(sim.key==='5145-hitch-low') return `${m} — Three-Point Hitch Raises Only About One Foot`;
  return sim.title;
}
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.4 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.4 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/student/:id/scenario', async(req,res)=>{
  ...
  const cards=assignments.map(a=>{
    ...
    return `<div class="card" style="border-top:5px solid var(--red)"><div class="section-title"><div><div class="eyebrow">${esc(sim.series)} · ${esc(sim.level)}</div><div class="big">${esc(displayTitle)}</div></div><span class="sim-badge">${esc(sim.workOrder)}</span></div><p><b>Tractor:</b> ${esc(tractorModel)} &nbsp; · &nbsp; <b>Hours:</b> ${esc(sim.hours)}</p><div class="alert"><b>Customer Complaint:</b><br>${esc(sim.complaint)}</div>${status}<div class="toolbar" style="margin-top:12px">${action}${review}</div></div>`;
  }).join('');
...
app.get('/instructor/simulations',auth,async(req,res)=>{
  ...
  const sims=Object.values(DIAGNOSTIC_SIMULATIONS).map(sim=>{
    const classSetup=classes.map(c=>{
      const a=amap.get(`${c.id}:${sim.key}`), selected=a?.root_cause_key||'', selectedTractor=a?.tractor_model||'';
      const tractors=tractorChoicesForSim(sim);
      return `<div style="border:1px solid #ddd;border-radius:12px;padding:14px;margin:10px 0"><div class="section-title"><div><b>${esc(c.course)}</b><div class="muted">Class ${esc(c.code)} · ${c.student_count} technician(s) · ${c.active?'Active':'Closed'}</div></div>${selected&&selectedTractor?'<span class="pill open">RELEASED</span>':'<span class="pill">NOT RELEASED</span>'}</div><form method="post" action="/instructor/simulations/${encodeURIComponent(sim.key)}/configure?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="margin-top:10px"><input type="hidden" name="class_id" value="${c.id}"><label>Tractor / Model<select name="tractor_model" required><option value="">Choose ONE tractor for this simulation</option>${tractors.map(m=>`<option value="${esc(m)}" ${selectedTractor===m?'selected':''}>${esc(m)}</option>`).join('')}</select></label><label>Planted Root Cause<select name="root_cause_key" required><option value="">Select the failure BEFORE students begin</option>${sim.rootCauses.map(r=>`<option value="${esc(r.key)}" ${selected===r.key?'selected':''}>${esc(r.diagnosis)}</option>`).join('')}</select></label><button>${selected&&selectedTractor?'Update Tractor / Root Cause':'Release This Simulation'}</button></form>${selected&&selectedTractor?`<div class="success" style="margin-top:8px"><b>Instructor Only:</b> Tractor: ${esc(selectedTractor)} · Root Cause: ${esc(rootCauseByKey(sim,selected)?.diagnosis||'')}</div>`:''}</div>`;
    }).join('')||'<p>No classes exist yet.</p>';
    return `<div class="card" style="border-top:5px solid var(--red)"><div class="eyebrow">SIMULATION SCENARIO</div><div class="big">${esc(sim.title)}</div><p><b>Available tractor choices:</b> ${tractorChoicesForSim(sim).map(esc).join(' · ')} &nbsp; · &nbsp; <b>Work Order:</b> ${esc(sim.workOrder)}</p><div class="alert"><b>Customer Complaint:</b> ${esc(sim.complaint)}</div><p><b>Instructor setup:</b> Choose exactly one tractor and one planted root cause for the class. Students will see only that tractor when they run the simulation.</p>${classSetup}<div class="toolbar"><a class="btn light" href="/instructor/simulations/${encodeURIComponent(sim.key)}/preview?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Preview Simulation & Root Causes</a></div></div>`;
  }).join('');
  res.send(layout('Diagnostic Simulations', `<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Command Center</a></div><div class="card" style="background:linear-gradient(135deg,#171717,#4b090d);color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">Diagnostic Failure Simulation System · v5.4</div><div class="big" style="font-size:30px">Simulation Library</div><p style="margin-bottom:0">Choose one tractor and one planted root cause before students can start.</p></div>${sims}<div class="card"><div class="big">Technician Attempts</div><p>Permanent record of each technician's diagnostic path, test sequence, parts-cannon penalties, final diagnosis, repair selection, score, and elapsed time.</p><div style="overflow:auto"><table class="sim-history"><thead><tr><th>Technician</th><th>Dealer</th><th>Simulation</th><th>Score</th><th>Penalty</th><th>Date</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="7">No technician has run a simulation yet.</td></tr>'}</tbody></table></div></div>`,simulationCss()));
});
