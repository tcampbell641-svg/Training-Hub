...
app.get('/instructor/class/:id/hunt-label-print',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found');
  res.send(layout('QR Label Printing', `<div class="toolbar"><a class="btn light" href="/instructor/hunt/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Back to Scavenger Hunt</a><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Class Dashboard</a></div><div class="card" style="max-width:650px;margin:0 auto"><div class="eyebrow">SCAVENGER HUNT</div><div class="big">QR Label Printing</div><p>Choose the label stock loaded in your T50M Pro, then open that print layout.</p><form method="get" action="/instructor/class/${c.id}/hunt-labels"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Label Size<select name="size" required><option value="" selected disabled>— Select Label Size —</option><option value="30">30 mm Compact</option><option value="40x60">40 × 60 mm</option><option value="50x80">50 × 80 mm</option><option value="50">Standard T50M / 50 mm</option></select></label><button type="submit" style="width:100%;margin-top:12px">Open Print Layout</button></form><hr><a class="btn light" style="width:100%;text-align:center" href="/instructor/class/${c.id}/hunt-qr?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Print Full QR Sheets</a><div class="alert" style="margin-top:14px"><b>Recommended:</b> Use Actual Size / 100% in the printer dialog. Test-scan the first label before printing the full set.</div></div>`));
});

app.get('/instructor/class/:id/hunt-labels',auth,async(req,res)=>{ const id=Number(req.params.id),size=String(req.query.size||'50x80'); const pin=encodeURIComponent(INSTRUCTOR_PIN); const routes={ '30':`/instructor/class/${id}/hunt-qr-t50m-30?pin=${pin}`, '40x60':`/instructor/class/${id}/hunt-qr-t50m-40x60?pin=${pin}`, '50x80':`/instructor/class/${id}/hunt-qr-t50m-50x80?pin=${pin}`, '50':`/instructor/class/${id}/hunt-qr-t50m?pin=${pin}` }; res.redirect(routes[size]||routes['50x80']); });

app.get('/instructor/class/:id/hunt-qr-t50m',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY tractor_no, station_order, id',[c.id]); const cfg=await pool.query('SELECT * FROM hunt_config WHERE class_id=$1',[c.id]);
  const count=Number(cfg.rows[0]?.tractor_count||3),target=Number(cfg.rows[0]?.items_per_tractor||5);
...
app.get('/instructor/class/:id/hunt-qr-t50m-50x80',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY tractor_no, station_order, id',[c.id]); const cfg=await pool.query('SELECT * FROM hunt_config WHERE class_id=$1',[c.id]);
  const count=Number(cfg.rows[0]?.tractor_count||3),target=Number(cfg.rows[0]?.items_per_tractor||5);
...
app.get('/instructor/class/:id/hunt-qr-t50m-40x60',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY tractor_no, station_order, id',[c.id]); const cfg=await pool.query('SELECT * FROM hunt_config WHERE class_id=$1',[c.id]);
  const count=Number(cfg.rows[0]?.tractor_count||3),target=Number(cfg.rows[0]?.items_per_tractor||5);
...
app.get('/instructor/class/:id/hunt-qr-t50m-30',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY tractor_no, station_order, id',[c.id]); const cfg=await pool.query('SELECT * FROM hunt_config WHERE class_id=$1',[c.id]);
  const count=Number(cfg.rows[0]?.tractor_count||3),target=Number(cfg.rows[0]?.items_per_tractor||5);
