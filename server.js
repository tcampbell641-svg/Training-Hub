...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.legacyIncomplete?`<div class="alert"><b>This attempt predates permanent quiz history.</b> The old database record did not contain the student's exact answers. No answers will be invented. New attempts are now permanently stored.</div>`:renderAttemptReview(repaired.review,old.score,true);
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
