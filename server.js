...
app.post('/student/:id/quiz', async(req,res)=>{
  ...
  const reviewHtml=`<div class="card" style="border-top:5px solid var(--red)"><div class="big">Quiz Review — Correct Answers</div><p class="muted">Every missed question is shown below with your answer and the correct answer.</p>${missed.length?missed.map((m,i)=>`<div class="q feedback-miss"><b>Missed Question ${i+1}${m.topic?' · '+esc(m.topic):''}</b><p><b>Question:</b> ${esc(m.question)}</p><p><b>Your Answer:</b> ${esc(m.selected)}</p><p><b>Correct Answer:</b> ${esc(m.correct)}</p><p><b>Explanation:</b> ${esc(m.explanation)}</p></div>`).join(''):'<div class="success"><b>Perfect score.</b> You did not miss any questions.</div>'}</div>`;
  ...
  res.send(layout('Quiz Complete', `<div class="card center"><div class="eyebrow">Training Hub v3.8</div><div class="big">Quiz Complete</div><div class="code">${score}%</div><p>${correct} of ${q.rowCount} correct</p></div>${certHtml}${reviewHtml}<div class="card center"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`));
});
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const result=q.rows[0], details=result.details||{}, missed=details.missed||[];
  const reviewHtml=missed.length?missed.map((m,i)=>`<div class="q feedback-miss"><b>Missed Question ${i+1}${m.topic?' · '+esc(m.topic):''}</b><p><b>Question:</b> ${esc(m.question||'')}</p><p><b>Your Answer:</b> ${esc(m.selected||'No answer')}</p><p><b>Correct Answer:</b> ${esc(m.correct||'')}</p><p><b>Explanation:</b> ${esc(m.explanation||'Review this topic with your instructor.')}</p></div>`).join(''):'<div class="success"><b>Perfect score.</b> You did not miss any questions.</div>';
  res.send(layout('Quiz Review', `<div class="card" style="border-top:5px solid var(--red)"><div class="eyebrow">Training Hub v3.8</div><div class="big">Quiz Review — Correct Answers</div><p>Your latest quiz score: <b>${result.score}%</b></p>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
function certificateMarkup(s, cert, startDate, endDate, backHref=''){
  ...
  return {
    ...
    css:`<style>
      ...
      .mahindra-cert{position:relative;background:#fff;max-width:1100px;min-height:760px;margin:0 auto;padding:42px 58px 34px;font-family:Georgia,'Times New Roman',serif;color:#111;box-shadow:0 5px 22px rgba(0,0,0,.16)}
      .cert-logo{position:absolute;left:54px;top:36px;width:235px}.cert-logo img{width:100%;height:auto;display:block}
      .cert-title{text-align:center;font-size:39px;font-weight:700;margin-top:32px}
      .cert-intro,.cert-success,.cert-location{font-style:italic;text-align:center;font-size:28px;margin-top:48px}
      .cert-fill{color:#0b43a0;text-align:center;font-weight:700;border-bottom:2px solid #222;padding:0 12px 7px}
      .cert-name{font-size:43px;margin:22px 70px 0}
      .cert-of-row{display:grid;grid-template-columns:55px 1fr;align-items:end;gap:10px;margin-top:45px}.cert-label{font-style:italic;font-size:28px}.cert-dealer{font-size:34px}
      .cert-success{margin-top:30px}
      .cert-course-row{display:flex;justify-content:center;align-items:flex-end;gap:18px;font-style:italic;font-size:27px;margin-top:24px;flex-wrap:wrap}.cert-course-fill{color:#0b43a0;font-style:normal;font-weight:700;font-size:31px;border-bottom:2px solid #222;min-width:460px;padding:0 10px 3px}
      .cert-location{margin-top:28px}
      .cert-dates{display:flex;justify-content:center;align-items:end;gap:14px;font-style:italic;font-size:25px;margin-top:38px;flex-wrap:wrap}.date-fill{color:#0b43a0;font-style:normal;font-weight:700;border-bottom:2px solid #222;padding:0 8px 4px;min-width:205px;text-align:center}
      .cert-signatures{display:grid;grid-template-columns:1fr 1fr;gap:210px;margin-top:68px;align-items:end}.sig-block{text-align:center;font-family:Arial,sans-serif;font-size:16px}.typed-signature{height:58px;display:flex;align-items:flex-end;justify-content:center;font-family:'Segoe Script','Brush Script MT',cursive;font-size:32px;font-style:italic}.manager-signature{display:block;height:58px;max-width:220px;object-fit:contain;margin:0 auto}.sig-line{border-top:2px solid #222;margin:2px auto 8px;max-width:320px}.sig-name{font-weight:700;font-size:18px}
      .cert-number{text-align:center;font-family:Arial,sans-serif;color:#666;font-size:11px;margin-top:22px}
      @media(max-width:800px){.mahindra-cert{padding:28px 20px;min-height:0}.cert-logo{position:static;width:180px;margin:0 auto 16px}.cert-title{font-size:28px;margin-top:0}.cert-intro,.cert-success,.cert-location{font-size:21px;margin-top:28px}.cert-name{font-size:32px;margin:16px 10px 0}.cert-of-row{grid-template-columns:35px 1fr;margin-top:28px}.cert-label{font-size:22px}.cert-dealer{font-size:25px}.cert-course-row{font-size:21px}.cert-course-fill{min-width:0;width:100%;font-size:25px}.cert-dates{font-size:20px}.cert-signatures{grid-template-columns:1fr;gap:36px;margin-top:48px}}
      @media print{@page{size:landscape;margin:.25in}.top,.no-print{display:none!important}.wrap{max-width:none!important;padding:0!important}.mahindra-cert{box-shadow:none;max-width:none;min-height:7.55in;padding:28px 48px 20px}.cert-number{margin-top:10px}}
    </style>`
  };
}
