// Mahindra Training Hub v5.7.7 — Video Answer Contrast Fix.6 — Fail-Safe Video Question Engine
...
app.get('/student/:id/video/:vid',async(req,res)=>{
  ...
  const extra=`<style>.video-shell{position:relative;background:#111;border-radius:14px;overflow:hidden}.video-shell video{display:block;width:100%;max-height:70vh;background:#000}.quiz-overlay{position:absolute;inset:0;background:rgba(0,0,0,.88);color:#fff;display:none;align-items:center;justify-content:center;padding:18px}.quiz-panel{width:min(720px,100%);background:#fff;color:#1d1d1d;border-radius:16px;padding:22px;max-height:90%;overflow:auto}.video-answer{display:block!important;width:100%!important;text-align:left!important;margin:9px 0!important;background:#f2f2f2!important;color:#1d1d1d!important;border:1px solid #cfcfcf!important;border-radius:10px!important;padding:13px 15px!important;font-weight:750!important;opacity:1!important}.video-answer:hover{background:#e4e4e4!important;color:#111!important}.video-answer.good{background:#dff3e4!important;color:#155d2d!important;border-color:#8fc99c!important}.video-answer.bad{background:#fff0a8!important;color:#4f3a00!important;border-color:#d6a800!important}.video-progressline{height:10px;background:#e5e5e5;border-radius:99px;overflow:hidden}.video-progressline span{display:block;height:100%;background:var(--red)}</style>`;
...
const questionWatchdog=setInterval(()=>{if(!active&&!video.ended)checkQuestionTrigger(false);},100);
let rafId=0;function questionFrame(){if(!active&&!video.ended)checkQuestionTrigger(false);rafId=requestAnimationFrame(questionFrame)};rafId=requestAnimationFrame(questionFrame);
