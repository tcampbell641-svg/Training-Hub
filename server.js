...
const DIAGNOSTIC_SIMULATIONS = {
  ...
  'can-network-down':{
    key:'can-network-down',series:'Electrical / CAN',level:'Level 3',title:'Multiple Modules Not Communicating',model:'Mahindra CAN-equipped Tractor',workOrder:'SIM-CAN-001',hours:521,complaint:'Multiple controllers are offline. Diagnostic tool cannot communicate with several modules.',assignment:'Use network resistance, voltage, isolation and power/ground testing to identify the failure.',
    rootCauses:[
...
const ALL_TRACTOR_MODELS = [
  ...
  '3100 HST','4100','4500','4600','5100','5155','5500',
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.5 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.5 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
