...
const LABEL_SPECS={
  '30':{label:'30 mm Compact',qr:650,canvasW:360,canvasH:480,nameChars:18,nameLines:2,descChars:22,descLines:2},
  '40x60':{label:'40 × 60 mm',qr:800,canvasW:420,canvasH:630,nameChars:20,nameLines:2,descChars:28,descLines:3},
  '50x80':{label:'50 × 80 mm',qr:1000,canvasW:520,canvasH:820,nameChars:24,nameLines:2,descChars:34,descLines:4},
  '50':{label:'Standard T50M / 50 mm',qr:900,canvasW:500,canvasH:620,nameChars:22,nameLines:2,descChars:30,descLines:3}
};
...
const HUNT_TYPES = {general:'General / Model Hunt',fmcu:'FMCU Electrical Deep Dive',auto4wd:'Auto 4WD Electrical Logic'};
function huntItemVisible(item,huntType,model){
  ...
  return !item.id.startsWith('refresh_') || is5100RefreshModel(model);
}
...
// Instructor-editable simulation content is stored in PostgreSQL so edits survive deploys/restarts.
...
function ensureSimulationDefaults(sim){
  ...
  sim.instructorSetup=sim.instructorSetup||'Set the tractor up so the customer complaint can be reproduced safely. Keep the planted failure hidden from the technician.';
  ...
  sim.resetProcedure=sim.resetProcedure||'Return the tractor to normal condition, clear/reset any related faults, and verify normal operation before the next technician.';
  sim.safetyNotes=sim.safetyNotes||'Follow normal shop safety procedures. Chock/secure the tractor as needed and do not bypass safety interlocks unless an approved training procedure specifically requires it.';
  sim.hints=Array.isArray(sim.hints)&&sim.hints.length?sim.hints:[
    ...
    'Compare physical tractor condition to live data, then choose the circuit that does not agree.',
...
function embeddedVideoKey(course=''){
  const c=String(course).toLowerCase().replace(/[^a-z0-9]+/g,' ');
  if(c.includes('4600')||c.includes('forty six hundred')) return '4600';
  if(c.includes('5100')||c.includes('fifty one hundred')) return '5100';
  if(c.includes('6000')||c.includes('six thousand')) return '6000';
  return '';
}
...
function embeddedVideoDefForRow(v){
  const hay=(String(v?.source_url||'')+' '+String(v?.title||'')).toLowerCase();
  let key='';
  if(hay.includes('4600_service_training')||hay.includes('4600 service training')||hay.includes('4600 introduction')) key='4600';
  else if(hay.includes('5100_refresh_introduction')||hay.includes('5100 refresh')||hay.includes('fifty one hundred')) key='5100';
  else if(hay.includes('6000_hce')||hay.includes('6000 hce')||hay.includes('six thousand')) key='6000';
  return key ? (EMBEDDED_VIDEO_CATALOG[key]||[])[0] : null;
}
...
async function init(){
  ...
  const defaults=['Forty Six Hundred Service Training','Fifty One Hundred Refresh','Six Thousand Series','3100','SU Series','ROXOR','Electrical Fundamentals','CAN / J1939 Diagnostics','FES / GARUDA Diagnostics'];
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.7.1 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.7.1 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/student/:id/hunt', async(req,res)=>{
  ...
  const groups=Array.from({length:tractorCount},(_,i)=>i+1).map(n=>{
    ...
    const body=rows.map(x=>{
      ...
      const action=pr?.correct?'<span class="muted">Completed</span>':`<a class="btn" href="/student/${s.id}/hunt/scan/${x.id}?token=${encodeURIComponent(s.join_token)}">Scan QR</a>`;
      return `<tr><td>${sn}</td><td><b>${esc(x.item_label||x.station_name)}</b></td><td>${status}</td><td>${action}</td></tr>`
    }).join('');
    return `<div class="card"><div class="section-title"><div><div class="eyebrow">TRACTOR ${n}</div><div class="big">${esc(tractorName(n))}</div></div><span class="pill">${rows.length} Item${rows.length===1?'':'s'}</span></div><div style="overflow:auto"><table><tr><th>#</th><th>Hunt Item</th><th>Status</th><th>Action</th></tr>${body||'<tr><td colspan="4">No hunt items assigned.</td></tr>'}</table></div></div>`
  }).join('');
  res.send(layout('Scavenger Hunt', `<div class="card"><div class="eyebrow">Hands-On Individual Activity</div><div class="big">Tractor Technician Scavenger Hunt</div><div class="grid"><div class="stat"><span>Your Progress</span><b>${correctCount} / ${q.rowCount}</b></div><div class="stat"><span>How It Works</span><b style="font-size:18px">Choose Item · Scan · Verify</b></div></div><div class="alert" style="color:#111"><b>How to use the hunt:</b> Tap <b>Scan QR</b> beside the item you are looking for, then scan the QR code at the tractor/component. The Training Hub will tell you immediately whether you found the correct station.</div></div>${groups}<div class="card"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`));
});
...
app.get('/student/:id/hunt/scan/:sid', async(req,res)=>{
  ...
  res.send(layout('Scan Hunt QR', `<div class="card center"><div class="eyebrow">SCAVENGER HUNT · READY TO SCAN</div><div class="big">${esc(station.item_label||station.station_name)}</div><p class="muted">${esc(station.tractor_model||'')} ${station.tractor_no?`· Tractor ${station.tractor_no}`:''}</p><div class="success"><b>Target selected.</b><br>Now use your phone Camera app to scan the QR code where you believe this item is located.</div><div class="alert" style="color:#111"><b>The Training Hub will check the QR code.</b><br>If it is the right station, you will see a green <b>Correct Station</b> message. If it is wrong, you will see a yellow <b>Wrong Station</b> message and can try again.</div><div class="toolbar" style="justify-content:center"><a class="btn light" href="/student/${s.id}/hunt?token=${encodeURIComponent(s.join_token)}">Back to Hunt List</a></div></div>`));
});
...
app.get('/student/:id/scenario/:attemptId', async(req,res)=>{
  ...
  const evidence=actions.length?actions.map((x,i)=>{
    ...
    const ev=x.evidence_text?`<div class="small" style="margin-top:5px"><b>Technician Evidence:</b> ${esc(x.evidence_text)}</div>`:'';
...
app.get('/instructor/dashboard',auth,async(req,res)=>{
  ...
  res.send(layout('Instructor Dashboard', `<div style="background:linear-gradient(135deg,#171717,#5a0b10);color:#fff;border-radius:22px;padding:28px;border-bottom:7px solid var(--red);box-shadow:0 8px 24px rgba(0,0,0,.12)"><div class="eyebrow" style="color:#ffb9bd">MAHINDRA TECHNICIAN TRAINING HUB · VERSION 5.7.3</div><h1 style="margin:7px 0 4px;font-size:36px">Instructor Command Center</h1><p style="margin:0;color:#eee">${esc(settings.home_message||'Start classes, build activities, watch technicians, and print training records.')}</p><div class="toolbar" style="margin-top:18px"><a class="btn" href="/instructor/new?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">+ Start New Class</a><a class="btn light" href="/instructor/history?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Student Records</a></div></div><div class="grid" style="margin-top:18px"><div class="stat"><span>LIVE CLASSES</span><b>${active}</b></div><div class="stat"><span>TECHNICIANS</span><b>${students}</b></div><div class="stat"><span>CERTIFICATES ISSUED</span><b>${certCount}</b></div><div class="stat green"><span>SYSTEM</span><b>ONLINE</b></div></div><div class="home-grid"><a class="home-card" href="/instructor/build-select?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">📝</div><div class="title">Tests</div><div class="desc">Build and manage quiz questions.</div></a><a class="home-card" href="/instructor/hunt-select?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">🔎</div><div class="title">Scavenger Hunt</div><div class="desc">Choose tractors, models, hunt items, and print QR stations.</div></a><a class="home-card" href="/instructor/simulations?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">🛠️</div><div class="title">Simulation Library</div><div class="desc">Release diagnostic scenarios, choose hidden root causes, and review technician diagnostic paths.</div></a><a class="home-card" href="/instructor/master-challenges?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="border-top:6px solid #171717"><div class="icon">🧠</div><div class="title">Master Diagnostic Challenge</div><div class="desc">Build, edit, release, and score final practical diagnostic certification challenges.</div></a><a class="home-card" href="/instructor/prepost?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">📈</div><div class="title">Pre/Post Improvement</div><div class="desc">Compare starting knowledge to final test performance by class, technician, and topic.</div></a><a class="home-card" href="/instructor/videos?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">🎬</div><div class="title">Interactive Videos</div><div class="desc">Built-in 4600, 5100 Refresh, and 6000 training videos with timed questions and Parts Cannon penalties.</div></a><a class="home-card" href="/instructor/reports?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">🏆</div><div class="title">Reports & Certificates</div><div class="desc">Print records and certificates for completed technicians.</div></a><a class="home-card" href="/instructor/feedback?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">★</div><div class="title">Training Feedback</div><div class="desc">Review technician ratings and comments.</div></a><a class="home-card" href="/instructor/admin?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">⚙️</div><div class="title">Admin & Content</div><div class="desc">Courses, wording, content, and training-system settings.</div></a></div><div class="card" style="border-top:5px solid var(--red)"><div class="section-title"><h2>Live & Recent Classes</h2><span class="pill open">v5.7.2 ACTIVE</span></div><div style="overflow:auto;margin-top:12px"><table><thead><tr><th>Class</th><th>Code</th><th>Students</th><th>Status</th><th>Action</th></tr></thead><tbody>${rows||'<tr><td colspan="5">No classes yet. Click Start New Class above.</td></tr>'}</tbody></table></div></div>`));
});
...
app.get('/instructor/simulations',auth,async(req,res)=>{
  ...
  const releases=(await pool.query(`SELECT sa.*,c.code,c.course FROM simulation_assignments sa JOIN classes c ON c.id=sa.class_id ORDER BY sa.configured_at DESC,sa.id DESC`)).rows;
  const releaseRows=releases.map(r=>{ const sim=simByKey(r.simulation_key); const title=sim?.title||r.simulation_key; return `<tr><td><b>${esc(title)}</b><br><span class="muted small">${esc(r.tractor_model||sim?.model||'')}</span></td><td><b>${esc(r.code)}</b><br><span class="muted small">${esc(r.course||'')}</span></td><td><span class="pill ${r.enabled?'open':'closed'}">${r.enabled?'ACTIVE':'INACTIVE'}</span></td><td class="nowrap"><form method="post" action="/instructor/simulations/${encodeURIComponent(r.simulation_key)}/class/${r.class_id}/${r.enabled?'deactivate':'activate'}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="display:inline"><button type="submit" class="${r.enabled?'danger':''}" onclick="return confirm('${r.enabled?'Deactivate this simulation for class '+esc(r.code)+'? Technicians will no longer see it, but prior attempts will stay saved.':'Reactivate this simulation for class '+esc(r.code)+'?'}')">${r.enabled?'Deactivate':'Reactivate'}</button></form></td></tr>`; }).join('');
  ...
  const sims=allSimulationKeys().map(key=>simByKey(key)).filter(Boolean).map(sim=>{
    ...
    return `<div class="card" style="border-top:5px solid var(--red)">
      ...
      <div class="toolbar"><a class="btn" href="/instructor/simulations/${encodeURIComponent(sim.key)}/edit?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Edit Simulation</a><a class="btn light" href="/instructor/simulations/${encodeURIComponent(sim.key)}/preview?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Preview</a><a class="btn light" href="/instructor/simulations/${encodeURIComponent(sim.key)}/setup-sheet?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Instructor Setup Sheet</a><form method="post" action="/instructor/simulations/${encodeURIComponent(sim.key)}/duplicate?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="display:inline"><button class="light" type="submit">Duplicate</button></form></div>
    </div>`;
  }).join('');
  res.send(layout('Diagnostic Simulations', `<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Command Center</a><a class="btn" href="/instructor/simulations/new?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">+ Create Simulation</a><a class="btn light" href="/instructor/simulations/live?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Live Progress</a></div><div class="card" style="background:linear-gradient(135deg,#171717,#4b090d);color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">Diagnostic Failure Simulation System · v5.7.1</div><div class="big" style="font-size:30px">Simulation Library</div><p style="margin-bottom:0">Choose the class, one tractor/model, and one hidden planted root cause. Releasing the simulation makes it available to every technician enrolled in that class.</p></div>${sims}<div class="card" style="border-top:5px solid var(--red)"><div class="section-title"><div><div class="eyebrow">RELEASE CONTROL</div><div class="big">Simulation Releases by Class</div></div><span class="pill">Activate / Deactivate</span></div><p class="muted">Deactivating a release hides it from technicians in that class. The simulation, setup, completed attempts, scores, and history are not deleted.</p><div style="overflow:auto"><table><thead><tr><th>Simulation</th><th>Class</th><th>Status</th><th>Action</th></tr></thead><tbody>${releaseRows||'<tr><td colspan="4">No simulations have been released yet.</td></tr>'}</tbody></table></div></div><div class="card"><div class="big">Technician Attempts</div><p>Permanent record of each technician's diagnostic path, test sequence, parts-cannon penalties, final diagnosis, repair selection, score, and elapsed time.</p><div style="overflow:auto"><table class="sim-history"><thead><tr><th>Technician</th><th>Dealer</th><th>Simulation</th><th>Score</th><th>Penalty</th><th>Date</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="7">No technician has run a simulation yet.</td></tr>'}</tbody></table></div></div>`,simulationCss()));
});


app.get('/instructor/simulations/new',auth,async(req,res)=>{
  res.send(layout('Create Simulation', `<div class="toolbar"><a class="btn light" href="/instructor/simulations?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Simulation Library</a></div><div class="card" style="border-top:6px solid var(--red)"><div class="eyebrow">CUSTOM FAILURE SIMULATION</div><div class="big">Create New Simulation</div><p>This creates a complete starter simulation that you can customize in the Simulation Editor.</p><form method="post" action="/instructor/simulations/new?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="grid"><label>Simulation Title<input name="title" required placeholder="Example: PTO Will Not Engage"></label><label>Series / Course<input name="series" placeholder="Example: 3100 Series"></label><label>Default Model<input name="model" placeholder="Example: 3100"></label><label>Work Order<input name="workOrder" placeholder="SIM-CUSTOM-001"></label></div><label>Customer Complaint<textarea name="complaint" rows="3" required></textarea></label><label>Student Assignment<textarea name="assignment" rows="3" required>Verify the complaint, follow the diagnostic path, prove the root cause, make only the supported repair, and verify the repair.</textarea></label><label>First Planted Root Cause<input name="diagnosis" required></label><label>Correct Repair / Verification<textarea name="repair" rows="3" required></textarea></label><button>Create Simulation & Open Editor</button></form></div>`));
});
...
app.post('/instructor/simulations/new',auth,async(req,res)=>{
  ...
  res.redirect(`/instructor/simulations/${encodeURIComponent(key)}/edit?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`);
});
...
app.post('/instructor/simulations/:key/duplicate',auth,async(req,res)=>{
  ...
  const clone=JSON.parse(JSON.stringify(source)), key='custom-'+Date.now().toString(36); clone.key=key; clone.title=source.title+' — Copy'; clone.workOrder=(source.workOrder||'SIM')+'-COPY';
...
app.post('/instructor/simulations/:key/add-root',auth,async(req,res)=>{
  ...
  res.redirect(`/instructor/simulations/${encodeURIComponent(req.params.key)}/edit?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`);
});

app.post('/instructor/simulations/:key/add-action',auth,async(req,res)=>{
  const sim=simByKey(req.params.key); if(!sim) return res.status(404).send('Simulation not found'); const next=JSON.parse(JSON.stringify(sim));
  const key='step_'+Date.now().toString(36); next.actions=next.actions||[]; next.actions.push({key,category:'Component Tests',label:'New Diagnostic Step',result:'Enter the result the technician should receive.',points:0,penalty:0,stage:3,evidenceRequired:true});
  await pool.query(`INSERT INTO simulation_content_overrides(simulation_key,content_json,updated_at) VALUES($1,$2::jsonb,now()) ON CONFLICT(simulation_key) DO UPDATE SET content_json=$2::jsonb,updated_at=now()`,[req.params.key,JSON.stringify(next)]); SIMULATION_OVERRIDE_CACHE[req.params.key]=next;
  res.redirect(`/instructor/simulations/${encodeURIComponent(req.params.key)}/edit?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`);
});
...
app.get('/instructor/simulations/live',auth,async(req,res)=>{
  ...
  for(const a of attempts){
    ...
    cards.push(`<div class="card" style="border-left:6px solid ${last?.feedback_type==='wrong'?'#d6a800':'#267a3f'}"><div class="section-title"><div><div class="eyebrow">CLASS ${esc(a.code)} · ATTEMPT #${a.id}</div><div class="big">${esc(a.name)} · ${esc(a.title)}</div></div><span class="sim-badge">Stage ${stage}/6</span></div><p>${esc(a.dealer)} · ${esc(a.tractor_model||'')} · ${esc(a.difficulty||'Intermediate')}</p><div class="grid"><div class="stat"><span>SCORE</span><b>${a.score}%</b></div><div class="stat"><span>PENALTY</span><b>-${a.penalty_points}</b></div><div class="stat"><span>HINTS</span><b>${hints}</b></div><div class="stat"><span>ELAPSED</span><b>${elapsedText(a.started_at)}</b></div></div><p><b>Last Step:</b> ${esc(last?.label||'Not started')}<br><span class="muted">${esc(last?.result_text||'Waiting for first diagnostic choice.')}</span></p><a class="btn light" href="/instructor/simulation/${a.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Open Technician Path</a></div>`);
  }
  res.send(layout('Live Simulation Progress', `<div class="toolbar"><a class="btn light" href="/instructor/simulations?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Simulation Library</a></div><div class="card" style="background:#171717;color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">LIVE INSTRUCTOR VIEW · REFRESHES EVERY 10 SECONDS</div><div class="big" style="font-size:30px">Simulation Progress</div><p>Watch the class diagnostic path, score, penalties, hints, elapsed time, and current stage.</p></div>${cards.join('')||'<div class="card"><div class="big">No Active Simulation Attempts</div><p>Technicians currently diagnosing will appear here automatically.</p></div>'}`,`${simulationCss()}<script>setTimeout(()=>location.reload(),10000)</script>`));
});

app.get('/instructor/simulations/:key/edit',auth,async(req,res)=>{
  const sim=simByKey(req.params.key); if(!sim) return res.status(404).send('Simulation not found');
  const rootCards=(sim.rootCauses||[]).map((r,ri)=>{
    const overrideRows=Object.entries(r.overrides||{}).map(([actionKey,value],oi)=>`<div class="card" style="padding:12px;margin:10px 0"><label>Result for ${esc(actionKey)}<textarea name="root_${ri}_override_${oi}" rows="3">${esc(value)}</textarea></label><input type="hidden" name="root_${ri}_override_key_${oi}" value="${esc(actionKey)}"></div>`).join('');
    const pointRows=Object.entries(r.points||{}).map(([actionKey,value],pi)=>`<div class="grid" style="grid-template-columns:1fr 140px;gap:10px"><label>Scored step<input value="${esc(actionKey)}" disabled></label><label>Points<input type="number" min="0" max="100" name="root_${ri}_point_${pi}" value="${Number(value)||0}"></label><input type="hidden" name="root_${ri}_point_key_${pi}" value="${esc(actionKey)}"></div>`).join('');
    return `<div class="card" style="border-left:5px solid var(--red)"><div class="big">Root Cause ${ri+1}</div><input type="hidden" name="root_${ri}_key" value="${esc(r.key)}"><input type="hidden" name="root_${ri}_override_count" value="${Object.keys(r.overrides||{}).length}"><input type="hidden" name="root_${ri}_point_count" value="${Object.keys(r.points||{}).length}"><label>Diagnosis<textarea name="root_${ri}_diagnosis" rows="2">${esc(r.diagnosis||'')}</textarea></label><label>Correct Repair<textarea name="root_${ri}_repair" rows="3">${esc(r.repair||'')}</textarea></label><label>Required proof-step keys (comma separated)<input name="root_${ri}_proof" value="${esc((r.proof||[]).join(', '))}"></label>${pointRows}<div class="eyebrow" style="margin-top:14px">ROOT-CAUSE-SPECIFIC TEST RESULTS</div>${overrideRows||'<p class="muted">No root-cause-specific result overrides.</p>'}</div>`;
  }).join('');
  const actionCards=(sim.actions||[]).map((a,i)=>`<div class="card" style="border-left:5px solid ${a.penalty?'#d8a900':'#777'}"><div class="big">Diagnostic Step ${i+1}</div><input type="hidden" name="action_${i}_key" value="${esc(a.key)}"><div class="grid"><label>Category<input name="action_${i}_category" value="${esc(a.category||'')}"></label><label>Button / Step Name<input name="action_${i}_label" value="${esc(a.label||'')}"></label></div><label>Default Result<textarea name="action_${i}_result" rows="3">${esc(a.result||'')}</textarea></label><div class="grid"><label>Base Points<input type="number" min="0" max="100" name="action_${i}_points" value="${Number(a.points)||0}"></label><label>Parts Cannon Penalty<input type="number" min="0" max="100" name="action_${i}_penalty" value="${Number(a.penalty)||0}"></label><label>Guided Stage (1-6)<input type="number" min="1" max="6" name="action_${i}_stage" value="${inferActionStage(a)}"></label><label>Evidence Required<select name="action_${i}_evidenceRequired"><option value="1" ${a.evidenceRequired?'selected':''}>Yes</option><option value="0" ${!a.evidenceRequired?'selected':''}>No</option></select></label></div><label>Why? Question<input name="action_${i}_whyPrompt" value="${esc(a.whyPrompt||'')}"></label><label>Correct Why Answer<input name="action_${i}_whyCorrect" value="${esc(a.whyCorrect||'')}"></label><label>Wrong Why Answer 1<input name="action_${i}_whyWrong1" value="${esc(a.whyWrong1||'')}"></label><label>Wrong Why Answer 2<input name="action_${i}_whyWrong2" value="${esc(a.whyWrong2||'')}"></label></div>`).join('');
  res.send(layout('Edit Simulation', `<div class="toolbar"><a class="btn light" href="/instructor/simulations?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Simulation Library</a><a class="btn light" href="/instructor/simulations/${encodeURIComponent(sim.key)}/preview?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Preview</a></div><div class="card" style="background:linear-gradient(135deg,#171717,#4b090d);color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">SIMULATION EDITOR</div><div class="big" style="font-size:30px">${esc(sim.title)}</div><p style="margin-bottom:0">Changes apply to this simulation for all future technician attempts.</p></div><form method="post" action="/instructor/simulations/${encodeURIComponent(sim.key)}/edit?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><input type="hidden" name="root_count" value="${sim.rootCauses.length}"><input type="hidden" name="action_count" value="${sim.actions.length}"><div class="card"><div class="big">Simulation Setup</div><div class="grid"><label>Title<input name="title" required value="${esc(sim.title||'')}"></label><label>Series<input name="series" value="${esc(sim.series||'')}"></label><label>Level<input name="level" value="${esc(sim.level||'')}"></label><label>Default Model Name<input name="model" value="${esc(sim.model||'')}"></label><label>Work Order<input name="workOrder" value="${esc(sim.workOrder||'')}"></label><label>Hour Meter<input type="number" min="0" name="hours" value="${Number(sim.hours)||0}"></label></div><label>Customer Complaint<textarea name="complaint" rows="4">${esc(sim.complaint||'')}</textarea></label><label>Student Assignment<textarea name="assignment" rows="3">${esc(sim.assignment||'')}</textarea></label><div class="grid"><label>Default Difficulty<select name="difficulty"><option ${sim.difficulty==='Basic'?'selected':''}>Basic</option><option ${sim.difficulty==='Intermediate'?'selected':''}>Intermediate</option><option ${sim.difficulty==='Advanced'?'selected':''}>Advanced</option></select></label><label>Default Passing Score<input type="number" min="0" max="100" name="passScore" value="${sim.passScore}"></label><label>Default Max Parts Cannon Penalties<input type="number" min="0" max="20" name="maxPartsPenalties" value="${sim.maxPartsPenalties}"></label></div></div><div class="card"><div class="big">Instructor Setup & Safety</div><label>Instructor Setup Notes<textarea name="instructorSetup" rows="3">${esc(sim.instructorSetup||'')}</textarea></label><label>Tools Needed<textarea name="toolsNeeded" rows="2">${esc(sim.toolsNeeded||'')}</textarea></label><label>How to Plant the Fault<textarea name="plantFault" rows="3">${esc(sim.plantFault||'')}</textarea></label><label>Expected Readings / Evidence<textarea name="expectedReadings" rows="3">${esc(sim.expectedReadings||'')}</textarea></label><label>Reset Procedure<textarea name="resetProcedure" rows="3">${esc(sim.resetProcedure||'')}</textarea></label><label>Safety Notes<textarea name="safetyNotes" rows="3">${esc(sim.safetyNotes||'')}</textarea></label></div><div class="card"><div class="big">Hint Levels</div><label>Hint 1 (small hint, -2)<textarea name="hint_0" rows="2">${esc(sim.hints?.[0]||'')}</textarea></label><label>Hint 2 (more specific, -4)<textarea name="hint_1" rows="2">${esc(sim.hints?.[1]||'')}</textarea></label><label>Hint 3 (strong direction, -6)<textarea name="hint_2" rows="2">${esc(sim.hints?.[2]||'')}</textarea></label></div><div class="card"><div class="section-title"><div><div class="big">Planted Root Causes</div><p class="muted">Edit the diagnosis, repair, proof steps, scoring, and the test results students see for each planted failure.</p></div><button type="submit" class="light" formmethod="post" formaction="/instructor/simulations/${encodeURIComponent(sim.key)}/add-root?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">+ Add Root Cause</button></div></div>${rootCards}<div class="card"><div class="section-title"><div><div class="big">Diagnostic Choices / Tools</div><p class="muted">These are the selections the technician makes while following the diagnostic path.</p></div><button type="submit" class="light" formmethod="post" formaction="/instructor/simulations/${encodeURIComponent(sim.key)}/add-action?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">+ Add Diagnostic Step</button></div></div>${actionCards}<div class="card"><button style="font-size:18px;padding:14px 24px">Save Simulation Changes</button>${DIAGNOSTIC_SIMULATIONS[sim.key]?`<a class="btn light" style="margin-left:8px" href="/instructor/simulations/${encodeURIComponent(sim.key)}/reset?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Reset to Factory Default</a>`:`<button type="submit" class="alt" formmethod="post" formaction="/instructor/simulations/${encodeURIComponent(sim.key)}/delete-custom?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="margin-left:8px">Delete Custom Simulation</button>`}</div></form>`,simulationCss()));
});
...
app.post('/instructor/simulations/:key/edit',auth,async(req,res)=>{
  ...
  for(let ri=0;ri<rootCount && ri<next.rootCauses.length;ri++){
    const r=next.rootCauses[ri]; r.diagnosis=clean(req.body[`root_${ri}_diagnosis`]); r.repair=clean(req.body[`root_${ri}_repair`]); r.proof=clean(req.body[`root_${ri}_proof`]).split(',').map(x=>x.trim()).filter(Boolean);
    const pc=Math.min(100,Math.max(0,Number(req.body[`root_${ri}_point_count`])||0)); const pts={};
    for(let pi=0;pi<pc;pi++){ const k=clean(req.body[`root_${ri}_point_key_${pi}`]); if(k) pts[k]=Math.max(0,Number(req.body[`root_${ri}_point_${pi}`])||0); } r.points=pts;
    const oc=Math.min(100,Math.max(0,Number(req.body[`root_${ri}_override_count`])||0)); const ovs={};
    for(let oi=0;oi<oc;oi++){ const k=clean(req.body[`root_${ri}_override_key_${oi}`]); if(k) ovs[k]=clean(req.body[`root_${ri}_override_${oi}`]); } r.overrides=ovs;
  }
  ...
  for(let i=0;i<actionCount && i<next.actions.length;i++){
    const a=next.actions[i]; a.category=clean(req.body[`action_${i}_category`]); a.label=clean(req.body[`action_${i}_label`]); a.result=clean(req.body[`action_${i}_result`]); a.points=Math.max(0,Number(req.body[`action_${i}_points`])||0); a.penalty=Math.max(0,Number(req.body[`action_${i}_penalty`])||0); a.stage=Math.min(6,Math.max(1,Number(req.body[`action_${i}_stage`])||inferActionStage(a))); a.evidenceRequired=req.body[`action_${i}_evidenceRequired`]==='1'; a.whyPrompt=clean(req.body[`action_${i}_whyPrompt`]); a.whyCorrect=clean(req.body[`action_${i}_whyCorrect`]); a.whyWrong1=clean(req.body[`action_${i}_whyWrong1`]); a.whyWrong2=clean(req.body[`action_${i}_whyWrong2`]);
  }
  ...
  res.redirect(`/instructor/simulations/${encodeURIComponent(req.params.key)}/edit?pin=${encodeURIComponent(INSTRUCTOR_PIN)}&saved=1`);
});
...
app.get('/instructor/hunt/:id',auth,async(req,res)=>{
 ...
 const tractorCards=Array.from({length:count},(_,i)=>i+1).map(n=>{
   ...
   const modelNote=huntType==='fmcu'?`<div class="alert" style="margin:10px 0"><b>FMCU Electrical Hunt:</b> Only FMCU electrical items are shown below.</div>`:huntType==='auto4wd'?`<div class="alert" style="margin:10px 0"><b>Auto 4WD Hunt:</b> Only Auto 4WD logic and electrical items are shown below.</div>`:is5100RefreshModel(model)?`<div class="alert" style="margin:10px 0"><b>Fifty One Hundred Refresh:</b> Refresh-specific hunt items are available below.</div>`:'';
...
app.get('/instructor/prepost',auth,async(req,res)=>{
  const classes=(await pool.query(`SELECT c.id,c.course,c.code,c.title,c.active,(SELECT count(*)::int FROM students s WHERE s.class_id=c.id) students FROM classes c ORDER BY c.active DESC,c.created_at DESC LIMIT 100`)).rows;
  const cards=classes.map(c=>`<div class="card" style="border-top:5px solid ${c.active?'var(--red)':'#333'}"><div class="eyebrow">CLASS ${esc(c.code)} · ${c.active?'LIVE':'CLOSED'}</div><div class="big">${esc(c.course)}</div><p>${esc(c.title||'')} · ${c.students} technician${c.students===1?'':'s'}</p><a class="btn" href="/instructor/class/${c.id}/prepost?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Open Improvement Dashboard</a></div>`).join('');
  res.send(layout('Pre/Post Improvement',`<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Command Center</a></div><div class="card" style="background:linear-gradient(135deg,#171717,#5a0b10);color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">KNOWLEDGE IMPROVEMENT · v5.7.1</div><div class="big" style="font-size:30px">Pre-Test vs Post-Test</div><p style="margin-bottom:0">First completed quiz = Pre-Test. Most recent later quiz = Post-Test.</p></div><div class="grid">${cards||'<div class="card">No classes yet.</div>'}</div>`));
});
...
app.get('/instructor/class/:id/prepost',auth,async(req,res)=>{
  ...
  res.send(layout('Pre/Post Improvement',`<div class="toolbar no-print"><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Live Class</a><a class="btn light" href="/instructor/prepost?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">All Classes</a><button onclick="window.print()">Print Dashboard</button></div><div class="card" style="background:linear-gradient(135deg,#171717,#5a0b10);color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">PRE-TEST VS POST-TEST</div><div class="big" style="font-size:30px">${esc(c.course)} · Class ${esc(c.code)}</div><p style="margin-bottom:0">Measures knowledge gain using each technician's first completed quiz and most recent later completed quiz.</p></div><div class="grid"><div class="stat"><span>PAIRED TECHNICIANS</span><b>${paired}</b></div><div class="stat"><span>PRE-TEST AVERAGE</span><b>${d.preAvg===null?'—':d.preAvg+'%'}</b></div><div class="stat green"><span>POST-TEST AVERAGE</span><b>${d.postAvg===null?'—':d.postAvg+'%'}</b></div><div class="stat"><span>AVERAGE IMPROVEMENT</span><b>${d.avgImprovement===null?'—':(d.avgImprovement>0?'+':'')+d.avgImprovement+' pts'}</b></div><div class="stat"><span>TECHNICIANS IMPROVED</span><b>${d.improved}/${paired}</b></div></div><div class="card"><div class="section-title"><div><div class="big">Technician Improvement</div><p class="muted">A technician needs at least two completed quiz attempts to produce a Pre/Post comparison.</p></div></div><div style="overflow:auto"><table><thead><tr><th>Technician</th><th>Pre-Test</th><th>Post-Test</th><th>Improvement</th><th>Status</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="6">No technicians enrolled yet.</td></tr>'}</tbody></table></div></div><div class="card"><div class="big">Topic Improvement</div><p class="muted">Aggregates correct-answer percentage by quiz topic from paired Pre-Test and Post-Test attempts.</p><div style="overflow:auto"><table><thead><tr><th>Topic</th><th>Pre-Test</th><th>Post-Test</th><th>Gain</th></tr></thead><tbody>${topicRows||'<tr><td colspan="4">Topic improvement appears after technicians complete both tests.</td></tr>'}</tbody></table></div></div><div class="alert"><b>Measurement rule:</b> The first completed quiz is locked as the Pre-Test. The newest later completed quiz is used as the Post-Test. Additional retakes update the Post-Test comparison but never replace the original Pre-Test.</div>`));
});
...
(function(){
  ...
  img.onload=()=>{
    ...
    const nameBlockHeight=(labelData.nameLines.length-1)*24;
...
app.get('/instructor/class/:id/hunt-qr-t50m-50x80',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY COALESCE(tractor_no,99),id',[c.id]);
  const count=Math.min(5,Math.max(1,Number(c.hunt_tractor_count)||3)),target=Math.min(12,Math.max(1,Number(c.hunt_items_per_tractor)||5));
  const counts=Array.from({length:count},(_,i)=>hs.rows.filter(x=>Number(x.tractor_no)===i+1).length); if(!counts.every(n=>n===target))return res.send(layout('Hunt Not Balanced',`<div class="card"><div class="big">Finish the scavenger hunt setup first</div><div class="alert">Every tractor must have exactly ${target} hunt items before 50 × 80 mm labels can be printed. ${counts.map((n,i)=>`Tractor ${i+1}: ${n}/${target}`).join(' · ')}</div><a class="btn" href="/instructor/hunt/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Back to Scavenger Hunt</a></div>`));
...
app.get('/instructor/class/:id/hunt-qr-t50m-30',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY COALESCE(tractor_no,99),id',[c.id]);
  const count=Math.min(5,Math.max(1,Number(c.hunt_tractor_count)||3)),target=Math.min(12,Math.max(1,Number(c.hunt_items_per_tractor)||5));
  const counts=Array.from({length:count},(_,i)=>hs.rows.filter(x=>Number(x.tractor_no)===i+1).length); if(!counts.every(n=>n===target))return res.send(layout('Hunt Not Balanced',`<div class="card"><div class="big">Finish the scavenger hunt setup first</div><div class="alert">Every tractor must have exactly ${target} hunt items before 30 mm labels can be printed. ${counts.map((n,i)=>`Tractor ${i+1}: ${n}/${target}`).join(' · ')}</div><a class="btn" href="/instructor/hunt/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Back to Scavenger Hunt</a></div>`));
...
function certificateMarkup(s, cert, startDate, endDate, backHref='', opts={}){
  ...
  const v={
    organization:opts.organization||'MAHINDRA Ag North America',
    ...
    location:opts.location||'Conducted at Mahindra AG North America,',
    ...
    instructorOrg:opts.instructorOrg||'Mahindra Ag North America',
    managerName:opts.managerName||'Nazar Mohamed',
    managerTitle:opts.managerTitle||'National Aftersales Manager',
    managerOrg:opts.managerOrg||'Mahindra Ag North America',
  ...
  return {
    ...
    <div class="cert-stage">
      <div class="mahindra-cert">
        <img class="cert-logo" style="transform:${certTransform(v.logoX,v.logoY)}" src="${MAHINDRA_LOGO_DATA_URI}" alt="Mahindra Rise">
        ...
        <div class="sig-block nazar-signature-block" style="text-align:${v.managerAlign};transform:${certTransform(v.managerX,v.managerY)}">
...
app.get('/instructor/student/:id/certificate/edit',auth,async(req,res)=>{
  ...
  const logoRow=`<div class="align-row"><div><b>Mahindra Logo</b></div><div></div><label>Move Left / Right<input type="number" min="-300" max="300" step="1" name="logoX" value="${esc(val('logoX','0'))}"></label><label>Move Up / Down<input type="number" min="-300" max="300" step="1" name="logoY" value="${esc(val('logoY','0'))}"></label></div>`;
  res.send(layout('Edit Certificate', `<div class="toolbar"><a class="btn light" href="/instructor/student/${st.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Student Record</a></div><form method="get" action="/instructor/student/${st.id}/certificate"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><div class="card"><div class="eyebrow">CERTIFICATE EDITOR</div><div class="big">Edit Before Printing</div><p class="muted">Change the certificate wording and alignment here. These changes affect only this printable preview and do not change the permanent student or class record.</p><div class="grid">${form('organization','Certificate Header / Organization',val('organization','MAHINDRA Ag North America'))}${form('intro','Intro Wording',val('intro','This is to certify that'))}${form('name','Technician Name',val('name',st.name))}${form('dealer','Dealer / Organization',val('dealer',st.dealer))}${form('success','Completion Wording',val('success','has successfully completed'))}${form('courseLabel','Training Label',val('courseLabel','Technical Training on'))}${form('course','Course / Training Name',val('course',st.course))}${form('location','Location / Conducted At',val('location','Conducted at Mahindra AG North America,'))}${form('startDate','Start Date',val('startDate',startDate))}${form('endDate','End Date',val('endDate',endDate))}${form('instructor','Instructor Name',val('instructor',st.instructor))}${form('instructorTitle','Instructor Title',val('instructorTitle','Instructor'))}${form('instructorOrg','Instructor Organization',val('instructorOrg','Mahindra Ag North America'))}${form('managerName','Manager Name',val('managerName','Nazar Mohamed'))}${form('managerTitle','Manager Title',val('managerTitle','National Aftersales Manager'))}${form('managerOrg','Manager Organization',val('managerOrg','Mahindra Ag North America'))}${form('certificateNo','Certificate Number',val('certificateNo',cert))}</div></div><div class="card"><div class="eyebrow">ALIGNMENT & POSITION</div><div class="big">Move Certificate Elements</div><p class="muted">Alignment changes text within its area. Position values move the element in pixels: negative horizontal values move left, positive move right; negative vertical values move up, positive move down.</p><div class="align-table">${logoRow}${alignRow('title','Certificate Header')}${alignRow('intro','Intro Wording')}${alignRow('name','Technician Name')}${alignRow('dealer','Dealer / Organization')}${alignRow('success','Completion Wording')}${alignRow('courseLabel','Training Label','left')}${alignRow('course','Course Name')}${alignRow('location','Location')}${alignRow('dates','Dates')}${alignRow('instructor','Instructor Signature Block')}${alignRow('manager','Manager Signature Block')}${alignRow('certNo','Certificate Number')}</div><div class="alert" style="margin-top:14px"><b>Tip:</b> Start with small adjustments such as 10 or -10. Preview the certificate, then click Edit Certificate again to fine-tune it.</div></div><div class="toolbar"><button>Preview Edited Certificate</button><button type="reset" class="btn light">Reset Form</button></div></form>`, `<style>.align-table{display:grid;gap:9px}.align-row{display:grid;grid-template-columns:minmax(180px,1.4fr) repeat(3,minmax(130px,1fr));gap:10px;align-items:end;border:1px solid #ddd;border-radius:10px;padding:10px;background:#fafafa}.align-row label{margin:0;font-size:13px}.align-row input,.align-row select{margin-top:3px}@media(max-width:850px){.align-row{grid-template-columns:1fr 1fr}.align-row>div:first-child{grid-column:1/-1}}</style>`));
});
...
app.get('/instructor/student/:id/certificate',auth,async(req,res)=>{
  ...
  const endDate=formatCertDate(quiz?.completed_at||new Date());
  const overrides=certOverrideFromQuery(req.query); const editQs=new URLSearchParams({pin:INSTRUCTOR_PIN,...overrides}).toString(); const certView=certificateMarkup(s,cert,startDate,endDate,'',{...overrides,editHref:`/instructor/student/${s.id}/certificate/edit?${editQs}`});
...
app.get('/instructor/videos',auth,async(req,res)=>{
  const q=await pool.query(`SELECT c.id,c.course,c.title,c.code,c.active,count(v.id)::int videos FROM classes c LEFT JOIN training_videos v ON v.class_id=c.id GROUP BY c.id ORDER BY c.created_at DESC LIMIT 50`);
  const rows=q.rows.map(c=>`<tr><td><b>${esc(c.course)}</b><br><span class="small muted">${esc(c.title)}</span></td><td>${esc(c.code)}</td><td>${c.videos}</td><td><a class="btn" href="/instructor/videos/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Manage Videos</a></td></tr>`).join('');
  res.send(layout('Interactive Videos', `<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Home</a></div><div class="card"><div class="big">Interactive Video Training</div><p class="muted">Choose a class. Built-in 4600, 5100 Refresh, and 6000 videos load automatically; you can also add Drive videos and place quiz questions at exact timestamps.</p><div style="overflow:auto"><table><tr><th>Course</th><th>Code</th><th>Videos</th><th></th></tr>${rows||'<tr><td colspan="4">No classes yet.</td></tr>'}</table></div></div>`));
});
...
app.get('/instructor/videos/:cid',auth,async(req,res)=>{
  ...
  for(const v of videos.rows){
    ...
    const qrows=qs.rows.map(q=>`<div class="q"><span class="pill">${fmtTime(q.timestamp_seconds)}</span> <b>${esc(q.question)}</b><div class="small muted" style="margin-top:6px">${q.choices.map((x,i)=>`${String.fromCharCode(65+i)}. ${esc(x)}${i===q.answer_index?' ✓':''}`).join(' · ')}</div><div class="small"><b>Retry:</b> ${q.require_correct?'Must answer correctly':'Continue after answer'} · <b>Parts Cannon:</b> ${q.parts_cannon_penalty||0}</div><form method="post" action="/instructor/videos/${c.id}/question/${q.id}/delete"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="danger" onclick="return confirm('Delete this video question?')">Delete Question</button></form></div>`).join('');
    cards.push(`<div class="card"><div class="section-title"><div><div class="eyebrow">Interactive Video</div><div class="big">${esc(v.title)}</div></div><span class="pill ${v.active?'open':'closed'}">${v.active?'Active':'Hidden'}</span></div><p class="small muted" style="word-break:break-all">${esc(v.source_url)}</p><div class="toolbar"><form method="post" action="/instructor/videos/${c.id}/video/${v.id}/toggle"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="btn light">${v.active?'Hide':'Show'} Video</button></form><form method="post" action="/instructor/videos/${c.id}/video/${v.id}/delete" onsubmit="return confirm('Delete this video and all of its in-video questions?')"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="danger">Delete Video</button></form></div><hr><div class="big" style="font-size:18px">Add In-Video Question</div><form method="post" action="/instructor/videos/${c.id}/video/${v.id}/question"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><div class="grid"><label>Minute<input type="number" name="minute" min="0" value="0" required></label><label>Second<input type="number" name="second" min="0" max="59" value="0" required></label></div><label>Question<textarea name="question" required></textarea></label>${['A','B','C','D'].map((x,i)=>`<label>${x}<input name="c${i}" required></label>`).join('')}<label>Correct Answer<select name="answer"><option value="0">A</option><option value="1">B</option><option value="2">C</option><option value="3">D</option></select></label><label>Feedback / Explanation<textarea name="explanation" rows="3"></textarea></label><div class="grid"><label>Parts Cannon Penalty<input type="number" min="0" name="penalty" value="0"></label><label><input style="width:auto" type="checkbox" name="require_correct" value="1" checked> Must answer correctly before video continues</label></div><button>Add Video Question</button></form>${qrows||'<p class="muted">No in-video questions yet.</p>'}</div>`);
  }
  res.send(layout('Video Manager', `<div class="toolbar"><a class="btn light" href="/instructor/videos?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← All Classes</a><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Open Class</a></div><div class="card"><div class="eyebrow">${esc(c.course)}</div><div class="big">Interactive Video Manager</div><p class="muted">Built-in course videos are preloaded from this deployment. You can also paste a Google Drive sharing link for additional MP4 training.</p><form method="post" action="/instructor/videos/${c.id}/video"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Video Title<input name="title" placeholder="Example: Module 1 — Introduction" required></label><label>Video Source<input name="source_url" placeholder="/videos/example.mp4 or https://drive.google.com/file/d/.../view" required></label><label>Display Order<input type="number" name="display_order" value="0"></label><button>Add Training Video</button></form></div>${cards.join('')||'<div class="card"><p>No videos have been added to this class yet.</p></div>'}`));
});
app.post('/instructor/videos/:cid/video',auth,async(req,res)=>{ const title=(req.body.title||'').trim(),url=(req.body.source_url||'').trim(); if(title&&url) await pool.query('INSERT INTO training_videos(class_id,title,source_url,display_order) VALUES($1,$2,$3,$4)',[req.params.cid,title,url,Number(req.body.display_order)||0]); res.redirect(`/instructor/videos/${req.params.cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
app.post('/instructor/videos/:cid/video/:vid/toggle',auth,async(req,res)=>{ await pool.query('UPDATE training_videos SET active=NOT active WHERE id=$1 AND class_id=$2',[req.params.vid,req.params.cid]); res.redirect(`/instructor/videos/${req.params.cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
app.post('/instructor/videos/:cid/video/:vid/delete',auth,async(req,res)=>{ await pool.query('DELETE FROM training_videos WHERE id=$1 AND class_id=$2',[req.params.vid,req.params.cid]); res.redirect(`/instructor/videos/${req.params.cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
app.post('/instructor/videos/:cid/video/:vid/question',auth,async(req,res)=>{ const t=(Math.max(0,Number(req.body.minute)||0)*60)+Math.max(0,Math.min(59,Number(req.body.second)||0)); await pool.query('INSERT INTO video_questions(video_id,timestamp_seconds,question,choices,answer_index,explanation,require_correct,parts_cannon_penalty) VALUES($1,$2,$3,$4,$5,$6,$7,$8)',[req.params.vid,t,req.body.question,JSON.stringify([req.body.c0,req.body.c1,req.body.c2,req.body.c3]),Number(req.body.answer),req.body.explanation||'',!!req.body.require_correct,Math.max(0,Number(req.body.penalty)||0)]); res.redirect(`/instructor/videos/${req.params.cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
app.post('/instructor/videos/:cid/question/:qid/delete',auth,async(req,res)=>{ await pool.query('DELETE FROM video_questions WHERE id=$1 AND video_id IN (SELECT id FROM training_videos WHERE class_id=$2)',[req.params.qid,req.params.cid]); res.redirect(`/instructor/videos/${req.params.cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
...
const questions=${JSON.stringify(data)}; let active=null,lastSave=0,lockedAt=null;
...
function showQuestion(q){
  ...
  // Snap back to the exact question point so a coarse browser timing event cannot visually skip it.
  if(Math.abs(video.currentTime-lockedAt)>0.75){try{video.currentTime=Math.max(0,lockedAt);}catch(e){}}
  ...
  q.choices.forEach((label,i)=>{const b=document.createElement('button');b.type='button';b.className='video-answer';b.textContent=String.fromCharCode(65+i)+'. '+label;b.addEventListener('click',()=>answer(q,i,b));choices.appendChild(b);});
}
function checkQuestionTrigger(){
  if(active||!questions.length)return;
  const q=nextPending();
  if(!q)return;
  const now=Number(video.currentTime)||0, target=Number(q.time)||0;
  // Trigger the earliest unanswered question as soon as playback reaches or passes it.
  if(now>=target-0.35){showQuestion(q);}
}
...
continueBtn.addEventListener('click',()=>{overlay.style.display='none';active=null;lockedAt=null;checkQuestionTrigger();if(!active)video.play().catch(()=>{});});
video.addEventListener('loadedmetadata',()=>{const saved=${Number(prog.last_second||0)};const pending=nextPending();let resume=saved;if(pending&&resume>Number(pending.time))resume=Math.max(0,Number(pending.time)-0.5);if(resume>1&&resume<video.duration-2)video.currentTime=resume;statusText.textContent='Resume position '+ftime(resume)+' · '+questions.filter(q=>q.done).length+' of '+questions.length+' questions completed';setTimeout(checkQuestionTrigger,100);});
video.addEventListener('timeupdate',()=>{if(video.duration){bar.style.width=Math.min(100,(video.currentTime/video.duration)*100)+'%';statusText.textContent=ftime(video.currentTime)+' / '+ftime(video.duration)+' · '+questions.filter(q=>q.done).length+' of '+questions.length+' questions completed';}checkQuestionTrigger();if(Date.now()-lastSave>5000){lastSave=Date.now();post({type:'progress',second:video.currentTime});}});
video.addEventListener('play',checkQuestionTrigger);
video.addEventListener('playing',checkQuestionTrigger);
video.addEventListener('seeked',checkQuestionTrigger);
video.addEventListener('ratechange',checkQuestionTrigger);
video.addEventListener('seeking',()=>{const p=nextPending();if(p&&!active&&video.currentTime>Number(p.time)+0.5){video.currentTime=Math.max(0,Number(p.time)-0.25);setTimeout(checkQuestionTrigger,50);}});
// Some browsers fire timeupdate sparsely. This watchdog guarantees the trigger is checked during playback.
const questionWatchdog=setInterval(()=>{if(!video.paused&&!video.ended)checkQuestionTrigger();},200);
window.addEventListener('beforeunload',()=>clearInterval(questionWatchdog));
