...
app.post('/student/:id/quiz-progress', async(req,res)=>{
  ...
  if(q) await pool.query(`INSERT INTO quiz_progress(student_id,question_id,is_correct,selected_answer,updated_at) VALUES($1,$2,$3,$4,now()) ON CONFLICT(student_id,question_id) DO UPDATE SET is_correct=$3,selected_answer=$4,updated_at=now()`,[s.id,Number(req.body.qid),isCorrect,Number(req.body.answer)]);
...
app.post('/student/:id/quiz', async(req,res)=>{
  ...
  for(const x of q.rows){
    ...
    if(ok) correct++;
    const item={question_id:x.id,question:x.question,topic:x.topic||'',selected_index:Number.isFinite(a)?a:null,answer_index:x.answer_index,selected:Number.isFinite(a)?(x.choices[a]||'No answer'):'No answer',correct:x.choices[x.answer_index],explanation:x.explanation||'Review this topic with your instructor.',is_correct:ok};
    review.push(item);
  ...
  res.send(layout('Quiz Complete', `<div class="card center"><div class="eyebrow">Training Hub v3.9.3</div><div class="big">Quiz Complete</div><div class="code">${score}%</div><p>${correct} of ${q.rowCount} correct</p><p class="muted">Questions missed: <b>${missed.length}</b></p></div>${certHtml}${reviewHtml}<div class="card center"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`));
});
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const repaired=await repairQuizReviewData(s.id,details);
  const reviewHtml=repaired.legacyIncomplete
    ? `<div class="card" style="border-top:5px solid var(--red)"><div class="big">Older Quiz Attempt</div><div class="alert"><b>The answer details for this older attempt were not saved.</b> Rather than show blank or incorrect information, take the quiz again. The new attempt will save the question, your answer, the correct answer, and the explanation.</div><div class="toolbar" style="margin-top:14px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a></div></div>`
    : renderQuizReview({review:repaired.review,missed:repaired.missed},{score:result.score,showAll:true});
  res.send(layout('Quiz Review', `<div class="card" style="border-top:5px solid var(--red)"><div class="eyebrow">Training Hub v3.9.3</div><div class="big">Quiz Review — Correct Answers</div><p>Your latest quiz score: <b>${result.score}%</b></p>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/instructor/student/:id',auth,async(req,res)=>{
  ...
  const quizResult=[...results.rows].reverse().find(x=>x.activity==='Module Quiz'); const reviewInfo=await repairQuizReviewData(s.id,quizResult?.details||{}); const missed=reviewInfo.missed; const quizReview=reviewInfo.legacyIncomplete?`<div class="alert"><b>This is an older quiz attempt.</b> The question and answer details were not stored in that attempt, so blank answers will not be displayed. Have the student retake the quiz to create a complete review.</div>`:missed.length?missed.map((m,i)=>`<div class="q feedback-miss"><b>Missed ${i+1}${m.topic?' · '+esc(m.topic):''}</b><p><b>Question:</b> ${esc(m.question||'')}</p><p><b>Student Answer:</b> ${esc(m.selected||'No answer')}</p><p><b>Correct Answer:</b> ${esc(m.correct||'')}</p><p><b>Explanation:</b> ${esc(m.explanation||'Review this topic with your instructor.')}</p></div>`).join(''):'<div class="success">No missed quiz questions recorded.</div>';
...
app.get('/instructor/student/:id/report',auth,async(req,res)=>{
  ...
  const quizResultReport=[...results.rows].reverse().find(x=>x.activity==='Module Quiz'); const reviewReport=await repairQuizReviewData(s.id,quizResultReport?.details||{}); const missedRows=reviewReport.legacyIncomplete?'<div class="alert">Older quiz attempt: detailed question and answer review was not stored. Student must retake the quiz for a complete review.</div>':reviewReport.missed.map((m,i)=>`<div class="q"><b>${i+1}. ${esc(m.question||'')}</b><p><b>Student Answer:</b> ${esc(m.selected||'No answer')}<br><b>Correct Answer:</b> ${esc(m.correct||'')}<br><b>Explanation:</b> ${esc(m.explanation||'Review this topic with your instructor.')}</p></div>`).join('');
...
async function repairQuizReviewData(studentId,details={}){
  ...
  const useful=info.review.filter(x=>x && String(x.question||'').trim() && String(x.correct||'').trim());
  if(useful.length){
    const review=info.review.map(x=>({question:x.question||'',topic:x.topic||'',selected:x.selected||'No answer',correct:x.correct||'',explanation:x.explanation||'Review this topic with your instructor.',is_correct:!!x.is_correct}));
    return {review,missed:review.filter(x=>!x.is_correct),legacyIncomplete:false};
  }
