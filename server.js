// Mahindra Training Hub v5.7 — Interactive Video Training
...
app.get('/instructor/videos',auth,async(req,res)=>{
  ...
  res.send(layout('Interactive Videos', `<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Home</a></div><div class="card"><div class="big">Interactive Video Training</div><p class="muted">Choose a class. Add its Google Drive training videos, then place quiz questions at exact timestamps.</p><div style="overflow:auto"><table><tr><th>Course</th><th>Code</th><th>Videos</th><th></th></tr>${rows||'<tr><td colspan="4">No classes yet.</td></tr>'}</table></div></div>`));
});
...
app.get('/instructor/videos/:cid',auth,async(req,res)=>{
  ...
  res.send(layout('Video Manager', `<div class="toolbar"><a class="btn light" href="/instructor/videos?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← All Classes</a><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Open Class</a></div><div class="card"><div class="eyebrow">${esc(c.course)}</div><div class="big">Google Drive Interactive Video Manager</div><p class="muted">Paste the normal Google Drive sharing link for the MP4. The Hub converts supported Drive links to a direct playback source.</p><form method="post" action="/instructor/videos/${c.id}/video"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Video Title<input name="title" placeholder="Example: Module 1 — Introduction" required></label><label>Google Drive Video Link<input name="source_url" placeholder="https://drive.google.com/file/d/.../view" required></label><label>Display Order<input type="number" name="display_order" value="0"></label><button>Add Training Video</button></form></div>${cards.join('')||'<div class="card"><p>No videos have been added to this class yet.</p></div>'}`));
});
...
function showQuestion(q){if(active)return;active=q;lockedAt=q.time;video.pause();overlay.style.display='flex';qText.textContent=q.question;timeBadge.textContent='Question at '+ftime(q.time);feedback.innerHTML='';continueBtn.style.display='none';choices.innerHTML='';q.choices.forEach((label,i)=>{const b=document.createElement('button');b.type='button';b.className='video-answer';b.textContent=String.fromCharCode(65+i)+'. '+label;b.addEventListener('click',()=>answer(q,i,b));choices.appendChild(b);});}
...
continueBtn.addEventListener('click',()=>{overlay.style.display='none';active=null;lockedAt=null;video.play().catch(()=>{});});
video.addEventListener('loadedmetadata',()=>{const saved=${Number(prog.last_second||0)};if(saved>1&&saved<video.duration-2)video.currentTime=saved;statusText.textContent='Resume position '+ftime(saved)+' · '+questions.filter(q=>q.done).length+' of '+questions.length+' questions completed';});
video.addEventListener('timeupdate',()=>{if(video.duration){bar.style.width=Math.min(100,(video.currentTime/video.duration)*100)+'%';statusText.textContent=ftime(video.currentTime)+' / '+ftime(video.duration)+' · '+questions.filter(q=>q.done).length+' of '+questions.length+' questions completed';}const q=questions.find(x=>!x.done&&video.currentTime>=x.time-0.2);if(q)showQuestion(q);if(Date.now()-lastSave>5000){lastSave=Date.now();post({type:'progress',second:video.currentTime});}});
video.addEventListener('seeking',()=>{const p=nextPending();if(p&&video.currentTime>p.time+1){video.currentTime=Math.max(0,p.time-0.25);}});
video.addEventListener('ended',async()=>{const pending=nextPending();if(pending){video.currentTime=Math.max(0,pending.time-0.25);showQuestion(pending);return;}await post({type:'complete',second:video.duration||video.currentTime});statusText.textContent='Video complete';bar.style.width='100%';setTimeout(()=>location.href='/student/${s.id}?token=${encodeURIComponent(s.join_token)}',700);});
