...
const DIAGNOSTIC_SIMULATIONS = {
  ...
  '3100-hst-no-forward':{
    key:'3100-hst-no-forward',series:'3100 HST',level:'Level 2',title:'3100 HST — No Forward Travel',model:'Mahindra 3100 HST',workOrder:'SIM-3100-HST-001',hours:286,complaint:'Tractor will move in reverse but will not travel forward. Engine operation is normal.',assignment:'Determine whether the failure is an input, controller, electrical output or hydraulic/mechanical problem.',
    rootCauses:[
  ...
  'can-network-down':{
    key:'can-network-down',series:'Electrical / CAN',level:'Level 3',title:'CAN Network — Multiple Modules Not Communicating',model:'Mahindra CAN-equipped Tractor',workOrder:'SIM-CAN-001',hours:521,complaint:'Multiple controllers are offline. Diagnostic tool cannot communicate with several modules.',assignment:'Use network resistance, voltage, isolation and power/ground testing to identify the failure.',
    rootCauses:[
  ...
  '6075-shuttle-reverse':{
    key:'6075-shuttle-reverse',series:'6000 / PST',level:'Level 3',title:'6075 Power Shuttle — Delayed / No Reverse',model:'Mahindra 6075 Power Shuttle',workOrder:'SIM-PST-001',hours:744,complaint:'Forward engagement is normal. Reverse is delayed and may fail to engage when hot.',assignment:'Separate electrical command, inching/interlock and hydraulic clutch-pressure causes before repair.',
    rootCauses:[
  ...
  '5145-hitch-low':{
    key:'5145-hitch-low',series:'5000 Series',level:'Level 2',title:'5155 — Three-Point Hitch Raises Only About One Foot',model:'Mahindra 5155',workOrder:'SIM-5155-001',hours:367,complaint:'Three-point hitch raises from the bottom but stops after approximately one foot of travel.',assignment:'Determine whether the limitation comes from command/calibration, feedback, hydraulics or mechanical linkage.',
    rootCauses:[
...
async function init(){
  ...
  const defaults=['Fifty One Hundred Refresh','Six Thousand Series','3100 HST','SU Series','ROXOR','Electrical Fundamentals','CAN / J1939 Diagnostics','FES / GARUDA Diagnostics'];
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.3.2 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.3.2 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/student/:id/scenario', async(req,res)=>{
  ...
  const cards=assignments.map(a=>{
    ...
    return `<div class="card" style="border-top:5px solid var(--red)"><div class="section-title"><div><div class="eyebrow">${esc(sim.series)} · ${esc(sim.level)}</div><div class="big">${esc(sim.title)}</div></div><span class="sim-badge">${esc(sim.workOrder)}</span></div><p><b>Model:</b> ${esc(sim.model)} &nbsp; · &nbsp; <b>Hours:</b> ${esc(sim.hours)}</p><div class="alert"><b>Customer Complaint:</b><br>${esc(sim.complaint)}</div>${status}<div class="toolbar" style="margin-top:12px">${action}${review}</div></div>`;
  }).join('');
