...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v4.3.3 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v4.3.3 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
function certificateMarkup(s, cert, startDate, endDate, backHref=''){
  ...
  return {
    ...
    css:`<style>
      ...
      .nazar-signature-block{right:95px;top:638px}
      ...
      .manager-signature{display:block;width:600px;height:164px;object-fit:contain;object-position:center;filter:brightness(.5) contrast(1.9);margin:0 auto -14px}
      .sig-line{border-top:2px solid #222;margin:4px auto 8px;width:320px;max-width:320px}
      ...
      @media print{
        ...
        .nazar-signature-block{right:.78in;top:5.58in}
        .typed-signature{height:.47in;font-size:24pt}
        .manager-signature{width:5.4in;height:1.44in;filter:brightness(.5) contrast(1.9)}
        .sig-line{width:2.85in;max-width:2.85in;margin:.04in auto .06in}
