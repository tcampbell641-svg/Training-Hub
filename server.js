...
function simByKey(key){ return editableSimulation(key); }
...
async function init(){
  ...
  for(const row of simOverrides.rows){
    if(DIAGNOSTIC_SIMULATIONS[row.simulation_key] && row.content_json){
...
app.get('/student/:id/scenario/:attemptId', async(req,res)=>{
  ...
  res.send(layout('Virtual Diagnostic Bay', `<div class="sim-shell"><aside class="sim-wo"><div class="eyebrow" style="color:#ffb9bd">VIRTUAL SHOP BAY</div><h2>${esc(a.tractor_model||sim.model)}</h2><div class="sim-meta"><div><span>Work Order</span><b>${esc(sim.workOrder)}</b></div><div><span>Hour Meter</span><b>${esc(sim.hours)} h</b></div><div><span>Status</span><b>Diagnosing</b></div><div><span>Elapsed</span><b id="simTimer" class="sim-timer">${elapsedText(a.started_at)}</b></div></div><div class="alert"><b>Complaint</b><br>${esc(sim.complaint)}</div><hr style="border-color:#444"><div>Current Score</div><div class="sim-score">${totals.score}%</div><div>Earned: ${totals.positive} · Penalties: -${totals.penalty}</div>${totals.penalty?`<div class="parts-penalty" style="margin-top:10px">Parts Cannon Penalty: -${totals.penalty}</div>`:''}</aside><main><div class="card sim-assignment"><div class="eyebrow">ASSOCIATED ASSIGNMENT</div><div class="big">Your Assignment</div><p style="font-size:17px;line-height:1.55;margin-bottom:14px">${esc(sim.assignment)}</p><div class="sim-assignment-steps"><div><b>1. Verify</b><span>Confirm the customer complaint under the stated operating conditions.</span></div><div><b>2. Diagnose</b><span>Follow the diagnostic path and use the available tests in a logical order.</span></div><div><b>3. Prove</b><span>Collect evidence that identifies the failed circuit, component, input, command, or mechanical condition.</span></div><div><b>4. Repair</b><span>Select the repair only after the failure has been proven. Unproven parts replacement can trigger a Parts Cannon penalty.</span></div><div><b>5. Verify Repair</b><span>Repeat the original complaint conditions and confirm the system now operates correctly.</span></div></div></div><div class="card"><div class="section-title"><div><div class="eyebrow">Choose Your Diagnostic Path</div><div class="big">Diagnostic Tools & Information</div><div class="sim-path-key"><span class="g"><i></i>Correct diagnostic path</span><span class="r"><i></i>Wrong / premature choice — Parts Cannon penalty</span><span class="n"><i></i>Information collected</span></div></div><span class="sim-badge">Attempt #${a.id}</span></div><div class="sim-tools" style="margin-top:15px">${tools}</div></div><div class="card"><div class="big">Evidence Log</div>${evidence}</div>${finalForm}</main></div>`, `${simulationCss()}<script>const st=new Date(${JSON.stringify(new Date(a.started_at).toISOString())}).getTime();setInterval(()=>{const x=Math.max(0,Math.floor((Date.now()-st)/1000)),m=Math.floor(x/60),ss=String(x%60).padStart(2,'0');const e=document.getElementById('simTimer');if(e)e.textContent=m+':'+ss;},1000);</script>`));
});
...
app.get('/instructor/simulations',auth,async(req,res)=>{
  ...
  const sims=Object.keys(DIAGNOSTIC_SIMULATIONS).map(key=>simByKey(key)).filter(Boolean).map(sim=>{
...
app.post('/instructor/simulations/:key/edit',auth,async(req,res)=>{
  ...
  for(let i=0;i<actionCount && i<next.actions.length;i++){
    const a=next.actions[i]; a.category=clean(req.body[`action_${i}_category`]); a.label=clean(req.body[`action_${i}_label`]); a.result=clean(req.body[`action_${i}_result`]); a.points=Math.max(0,Number(req.body[`action_${i}_points`])||0); a.penalty=Math.max(0,Number(req.body[`action_${i}_penalty`])||0);
  }
