...
const HUNT_ITEM_LIBRARY = [
  ...
  {id:'cluster',label:'Instrument Cluster',task:'Identify the instrument cluster and major warning/indicator area. Enter CLUSTER when complete.',expected:'cluster'},
  ...
  {id:'refresh_battery_air',label:'5100 Refresh — Battery & Air Cleaner Relocation',task:'Find the battery and air cleaner in their Refresh locations and compare them with the earlier Fifty One Hundred. Enter FRONT when complete.',expected:'front'},
...
function huntTeamInfo(rosterIndex,tractorCount){
  ...
  const rotation=Array.from({length:tractorCount},(_,i)=>((teamIndex+i)%tractorCount)+1);
  return {teamIndex,memberSlot,name:HUNT_TEAM_NAMES[teamIndex]||`TEAM ${teamIndex+1}`,rotation};
}
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.7 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.7 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/student/:id/hunt', async(req,res)=>{
  ...
  const team=huntTeamInfo(rosterIndex,tractorCount);
  ...
  const p=await pool.query('SELECT station_id,correct FROM hunt_progress WHERE student_id=$1',[s.id]); const completed=new Map(p.rows.map(x=>[Number(x.station_id),x]));
  ...
  const rotation=team.rotation.map(n=>`Tractor ${n} — ${esc(models.get(n)||'Model not set')}`).join(' → ');
  const groups=[];
  for(let n=1;n<=tractorCount;n++){
    const stations=q.rows.filter(x=>Number(x.tractor_no||1)===n);
    const rows=stations.map(x=>{ const globalIndex=q.rows.findIndex(y=>y.id===x.id)+1,role=huntRoleForStation(team.memberSlot,globalIndex); return `<tr><td>${globalIndex}</td><td><b>${esc(x.item_label||x.station_name)}</b><br><span class="small muted">Your role: ${esc(role.name)}</span></td><td>${completed.has(Number(x.id))?'<span class="pill results">Complete</span>':'<span class="pill">Not Scanned</span>'}</td></tr>`; }).join('');
    groups.push(`<div class="card"><div class="eyebrow">TRACTOR ${n}</div><div class="big">${esc(models.get(n)||'Model not set')}</div>${rows?`<table><tr><th>#</th><th>Hunt Item / Your Role</th><th>Status</th></tr>${rows}</table>`:'<div class="alert">No hunt items have been assigned to this tractor yet.</div>'}</div>`);
  }
  res.send(layout('Scavenger Hunt', `<div class="card" style="border-top:5px solid var(--red)"><div class="eyebrow">CUSTOM TRACTOR SCAVENGER HUNT · v5.8</div><div class="big">${esc(team.name)}</div><p><b>Your starting tractor:</b> Tractor ${team.rotation[0]} — ${esc(models.get(team.rotation[0])||'Model not set')}</p><p><b>Your rotation:</b> ${rotation}</p><p>Your job changes automatically at every station. Follow the role shown on your phone when you scan each QR code.</p><div class="stat"><span>Progress</span><b>${completed.size} / ${q.rowCount}</b><div class="progressbar"><span style="width:${q.rowCount?Math.round(completed.size/q.rowCount*100):0}%"></span></div></div></div>${q.rowCount?groups.join(''):'<div class="card"><div class="big">Waiting for Instructor</div><p>No scavenger-hunt stations have been configured for this class yet.</p></div>'}<div class="toolbar"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`));
});
...
app.get('/hunt-station/:cid/:sid', async(req,res)=>{
  ...
  const ordered=(await pool.query('SELECT id FROM hunt_stations WHERE class_id=$1 ORDER BY COALESCE(tractor_no,99),id',[station.class_id])).rows;
  const stationNumber=Math.max(1,ordered.findIndex(x=>Number(x.id)===Number(station.id))+1);
  ...
  const roleBox=`<div class="alert" style="border-left:6px solid var(--red)"><div class="eyebrow">YOUR ROLE AT THIS STATION</div><div class="big">${esc(role.name)}</div><p>${esc(role.duty)}</p></div>`;
  res.send(layout(station.station_name, `<div class="card"><span class="pill">${esc(team.name)} · Tractor ${station.tractor_no||'—'}</span><div class="big" style="margin-top:10px">${esc(station.item_label||station.station_name)}</div><p class="muted">${esc(station.tractor_model||'')}</p>${roleBox}<p><b>Task:</b> ${esc(station.task)}</p>${done?`<div class="success">Station already completed. Your answer: <b>${esc(done.answer)}</b></div><p><b>Next role:</b> ${esc(nextRole.name)}</p><a class="btn light" href="/student/${student.id}/hunt?token=${encodeURIComponent(station.join_token)}">View Hunt Progress</a>`:`<form method="post" action="/hunt-station/${station.class_id}/${station.id}"><input type="hidden" name="student_id" value="${student.id}"><input type="hidden" name="token" value="${esc(station.join_token)}"><label>Your Answer / Measurement<input name="answer" required autofocus></label><button>Submit Station</button></form><p class="small muted">After this station your next role will be <b>${esc(nextRole.name)}</b>.</p>`}</div>`));
});
...
app.get('/instructor/builder/:id',auth,async(req,res)=>{
  ...
  const tq=await pool.query('SELECT tractor_no,model FROM hunt_tractors WHERE class_id=$1 ORDER BY tractor_no',[c.id]);
  const models=new Map(tq.rows.map(x=>[Number(x.tractor_no),x.model]));
  const count=Math.min(HUNT_MAX_TRACTORS,Math.max(1,Number(c.hunt_tractor_count)||3));
  const qcards=qs.rows.map((q,i)=>`<div class="q"><b>${i+1}. ${esc(q.question)}</b><div class="small muted">${esc(q.topic||'General')}</div><form method="post" action="/instructor/builder/${c.id}/question/${q.id}/delete" style="margin-top:8px"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="danger" onclick="return confirm('Delete this question?')">Delete Question</button></form></div>`).join('');
  const tractorCards=Array.from({length:count},(_,i)=>{
    const n=i+1, model=models.get(n)||'';
    const current=hs.rows.filter(h=>Number(h.tractor_no)===n);
    const selectedLabels=new Set(current.map(h=>h.item_label));
    const options=HUNT_ITEM_LIBRARY.map(item=>`<label style="display:flex;gap:8px;align-items:flex-start"><input style="width:auto;margin-top:4px" type="checkbox" name="items" value="${esc(item.id)}" ${selectedLabels.has(item.label)?'disabled':''}><span><b>${esc(item.label)}</b><br><span class="small muted">${esc(item.task)}</span>${selectedLabels.has(item.label)?'<br><span class="pill results">Already Added</span>':''}</span></label>`).join('');
    const currentRows=current.map(h=>`<div class="q"><b>${esc(h.item_label||h.station_name)}</b><div class="small muted">${esc(h.task)}</div><div class="small"><b>Expected:</b> ${esc(h.expected)}</div><form method="post" action="/instructor/builder/${c.id}/station/${h.id}/delete" style="margin-top:8px"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="danger" onclick="return confirm('Delete this hunt item?')">Delete Hunt Item</button></form></div>`).join('');
    return `<div class="card" style="border-top:5px solid var(--red)"><div class="eyebrow">TRACTOR ${n}</div><div class="big">${esc(model||'Choose Model')}</div><form method="post" action="/instructor/builder/${c.id}/hunt/tractor/${n}/model"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Tractor Model<input name="model" value="${esc(model)}" placeholder="Example: Fifty One Hundred Refresh" required></label><button>Save Tractor Model</button></form><hr><div class="big" style="font-size:20px">Choose Hunt Items</div><form method="post" action="/instructor/builder/${c.id}/hunt/tractor/${n}/items"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><div style="display:grid;gap:10px;max-height:520px;overflow:auto;padding-right:6px">${options}</div><button style="margin-top:12px">Add Selected Items</button></form><hr><div class="big" style="font-size:20px">Custom Hunt Item</div><form method="post" action="/instructor/builder/${c.id}/station"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><input type="hidden" name="tractor_no" value="${n}"><label>Item Name<input name="name" required placeholder="Example: Shuttle pressure test port"></label><label>Technician Task<textarea name="task" required></textarea></label><label>Expected Answer / Verification<input name="expected" required></label><button>Add Custom Hunt Item</button></form><hr><p><b>${current.length}</b> item(s) assigned to Tractor ${n}.</p>${currentRows||'<div class="muted">No hunt items assigned yet.</div>'}</div>`;
  }).join('');
  res.send(layout('Training Builder', `<div class="toolbar"><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Back to Class</a><a class="btn" target="_blank" href="/instructor/class/${c.id}/hunt-qr?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Print Hunt QR Codes</a></div><div class="card" style="border-top:6px solid var(--red)"><div class="eyebrow">SCAVENGER HUNT BUILDER · v5.8</div><div class="big">Choose Your Tractors & Hunt Items</div><p>Select how many tractors you will use, name each tractor model, then choose exactly what the students must find.</p><form method="post" action="/instructor/builder/${c.id}/hunt/tractor-count"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>How Many Tractors Will You Use?<select name="tractor_count">${[1,2,3,4,5].map(n=>`<option value="${n}" ${n===count?'selected':''}>${n} Tractor${n===1?'':'s'}</option>`).join('')}</select></label><button>Save Tractor Count</button></form><form method="post" action="/instructor/builder/${c.id}/hunt/clear" style="margin-top:10px"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="danger" onclick="return confirm('Clear ALL scavenger-hunt stations for this class?')">Clear All Hunt Stations</button></form></div><div class="grid">${tractorCards}</div><div class="card"><div class="big">Add Quiz Question</div><form method="post" action="/instructor/builder/${c.id}/question"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Topic / Module<input name="topic" placeholder="Example: CAN / J1939"></label><label>Question<textarea name="question" required></textarea></label>${['A','B','C','D'].map((x,i)=>`<label>${x}<input name="c${i}" required></label>`).join('')}<label>Correct Answer<select name="answer"><option value="0">A</option><option value="1">B</option><option value="2">C</option><option value="3">D</option></select></label><label>Explanation / Student Feedback<textarea name="explanation" rows="3"></textarea></label><button>Add Question</button></form><p>${qs.rowCount} question(s) currently saved.</p>${qcards}</div>`));
});
...
app.get('/instructor/class/:id/hunt-qr',auth,async(req,res)=>{
  ...
  const origin=`${req.protocol}://${req.get('host')}`,cards=[];
  for(let i=0;i<hs.rows.length;i++){ const h=hs.rows[i],url=`${origin}/hunt-station/${c.id}/${h.id}`,qr=await QRCode.toDataURL(url,{width:320,margin:1}),n=Number(h.tractor_no)||0,model=h.tractor_model||models.get(n)||`Tractor ${n}`; cards.push(`<div class="card center" style="break-inside:avoid"><div class="eyebrow">TRACTOR ${n} · ${esc(model)}</div><h2>Station ${i+1}</h2><h3>${esc(h.item_label||h.station_name)}</h3><img class="qr" src="${qr}" alt="QR code for ${esc(h.station_name)}"><p>${esc(h.task)}</p><p class="small muted">Class ${esc(c.code)} · ${esc(c.course)}</p></div>`); }
  res.send(layout('Scavenger Hunt QR Codes', `<div class="no-print toolbar"><button onclick="window.print()">Print QR Station Sheets</button><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Back to Class</a></div><div class="center"><h1>${esc(c.course)} — Custom Tractor Scavenger Hunt</h1><p>Print these sheets and place each one at the tractor/model shown on the card.</p></div><div class="grid">${cards.join('')||'<div class="card">No hunt items have been configured yet.</div>'}</div>`, `<style>@media print{.grid{grid-template-columns:1fr 1fr}.card{border:1px solid #999!important;padding:16px!important}.qr{max-width:220px}}</style>`));
});
