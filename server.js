...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.0 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.0 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/instructor/simulations',auth,async(req,res)=>{
  ...
  res.send(layout('Diagnostic Simulations', `<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Command Center</a></div><div class="card"><div class="eyebrow">Diagnostic Failure Simulation System · v5.0</div><div class="big">Simulation Attempts</div><p>Permanent record of the technician's diagnostic path, test sequence, parts-cannon penalties, final diagnosis, repair selection, score, and elapsed time.</p><div style="overflow:auto"><table class="sim-history"><thead><tr><th>Technician</th><th>Dealer</th><th>Simulation</th><th>Score</th><th>Penalty</th><th>Date</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="7">No simulation attempts yet.</td></tr>'}</tbody></table></div></div>`,simulationCss()));
});
