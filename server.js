...
app.get('/student/:id', async(req,res)=>{
  ...
  const simAssignmentsAll=(await pool.query(`SELECT simulation_key,root_cause_key,technician_limit FROM simulation_assignments WHERE class_id=$1 AND enabled=true`,[s.class_id])).rows;
  const simRoster=(await pool.query('SELECT id FROM students WHERE class_id=$1 ORDER BY joined_at,id',[s.class_id])).rows;
  const simRosterPos=Math.max(1,simRoster.findIndex(x=>Number(x.id)===Number(s.id))+1);
  const simAssignments=simAssignmentsAll.filter(x=>simRosterPos<=Math.max(1,Math.min(15,Number(x.technician_limit)||15)));
...
app.get('/student/:id/scenario', async(req,res)=>{
  const s=await studentContext(req.params.id,req.query.token); if(!s) return res.status(403).send('Invalid session');
  const assignmentsAll=(await pool.query(`SELECT * FROM simulation_assignments WHERE class_id=$1 AND enabled=true ORDER BY configured_at DESC`,[s.class_id])).rows;
  const roster=(await pool.query('SELECT id FROM students WHERE class_id=$1 ORDER BY joined_at,id',[s.class_id])).rows;
  const rosterPos=Math.max(1,roster.findIndex(x=>Number(x.id)===Number(s.id))+1);
  const assignments=assignmentsAll.filter(a=>rosterPos<=Math.max(1,Math.min(15,Number(a.technician_limit)||15)));
...
app.post('/student/:id/scenario/start', async(req,res)=>{
  ...
  const roster=(await pool.query('SELECT id FROM students WHERE class_id=$1 ORDER BY joined_at,id',[s.class_id])).rows;
  const rosterPos=Math.max(1,roster.findIndex(x=>Number(x.id)===Number(s.id))+1);
  const technicianLimit=Math.max(1,Math.min(15,Number(assignment.technician_limit)||15));
  if(rosterPos>technicianLimit) return res.status(403).send('This simulation is currently released to '+technicianLimit+' technician(s) in this class.');
...
app.get('/instructor/simulations',auth,async(req,res)=>{
  ...
  const sims=Object.values(DIAGNOSTIC_SIMULATIONS).map(sim=>{
    ...
    const selectedTechnicians=Math.max(1,Math.min(15,Number(a?.technician_limit)||15));
    ...
    return `<div class="card" style="border-top:5px solid var(--red)">
      ...
      <form method="post" action="/instructor/simulations/${encodeURIComponent(sim.key)}/configure?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="margin-top:14px">
        ...
        <label>Technicians<select name="technician_limit" required>${Array.from({length:15},(_,i)=>i+1).map(n=>`<option value="${n}" ${selectedTechnicians===n?'selected':''}>${n} technician${n===1?'':'s'}</option>`).join('')}</select></label>
      ...
      ${selectedClass&&selectedRoot&&selectedTractor?`<div class="success" style="margin-top:10px"><b>Instructor Only:</b> Class ${esc(selectedClassRow?.code||selectedClass)} · Tractor: ${esc(selectedTractor)} · Technicians: ${selectedTechnicians} · Root Cause: ${esc(rootCauseByKey(sim,selectedRoot)?.diagnosis||'')}</div>`:''}
...
app.post('/instructor/simulations/:key/configure',auth,async(req,res)=>{
  ...
  const classId=Number(req.body.class_id), rootKey=(req.body.root_cause_key||'').trim(), tractorModel=(req.body.tractor_model||'').trim(), technicianLimit=Math.max(1,Math.min(15,Number(req.body.technician_limit)||1)); if(!classId||!rootCauseByKey(sim,rootKey)||!validTractorForSim(sim,tractorModel)) return res.status(400).send('Choose a valid class, tractor, technician count, and root cause.');
  ...
  await pool.query(`INSERT INTO simulation_assignments(class_id,simulation_key,root_cause_key,tractor_model,technician_limit,enabled,configured_at) VALUES($1,$2,$3,$4,$5,true,now()) ON CONFLICT(class_id,simulation_key) DO UPDATE SET root_cause_key=$3,tractor_model=$4,technician_limit=$5,enabled=true,configured_at=now()`,[classId,sim.key,rootKey,tractorModel,technicianLimit]);
