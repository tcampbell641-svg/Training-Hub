...
const HUNT_MODEL_SUGGESTIONS = [
  'Fifty One Hundred Refresh','Fifty One Hundred','Six Thousand Series','Forty Six Hundred','Forty One Hundred','Thirty One Hundred',
  'OJA 1100','OJA 2100','SU Series','ROXOR','1600 Series','2600 Series','4500 Series','Custom Tractor'
];
const HUNT_ITEM_LIBRARY = [
  ...
  {id:'refresh_battery_air',label:'5100 Refresh Change — Battery & Air Cleaner Relocation',task:'Find the battery and air cleaner in their Refresh locations and discuss how those locations differ from the earlier Fifty One Hundred. Enter FRONT when complete.',expected:'front'},
  {id:'refresh_app',label:'5100 Refresh Change — Accelerator Pedal Sensor Mounting',task:'Locate the accelerator-pedal-position sensor and identify the revised mounting location. Enter APP when complete.',expected:'app'},
  {id:'refresh_cooling',label:'5100 Refresh Change — Water Pump / Crank Pulley',task:'Locate the modified water-pump area and changed crank pulley. Enter COOLING when both have been identified.',expected:'cooling'},
  {id:'refresh_exhaust',label:'5100 Refresh Change — DOC / Exhaust Arrangement',task:'Locate the integrated DOC/exhaust arrangement and identify the changed service layout. Enter DOC when complete.',expected:'doc'},
  {id:'refresh_oil_cooler',label:'5100 Refresh Change — Oil Cooler Location',task:'Locate the revised oil-cooler position. Enter COOLER when complete.',expected:'cooler'},
  {id:'refresh_hyd_inlet',label:'5100 Refresh Change — Hydraulic Pump Inlet Pipe',task:'Locate the modified hydraulic-pump inlet-pipe area and identify the improved pump mounting-bolt access. Enter INLET when complete.',expected:'inlet'},
  {id:'refresh_egr',label:'5100 Refresh Change — EGR Mounting',task:'Locate the EGR assembly and identify the revised mounting arrangement. Enter EGR when complete.',expected:'egr'},
  {id:'refresh_oil_fill',label:'5100 Refresh Change — Engine Oil Fill Location',task:'Locate the revised engine-oil fill location. Enter OIL when complete.',expected:'oil'}
];
const HUNT_ROLES = [
  {name:'Navigator',duty:'Read the station task and use service information to guide the team.'},
...
function huntTeamInfo(rosterIndex,tractorCount=3){
  ...
  return {name:HUNT_TEAM_NAMES[teamIndex]||`TEAM ${teamIndex+1}`,teamIndex,memberSlot,rotation};
}
...
async function init(){
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS hunt_tractors(
    ...
    tractor_no INTEGER NOT NULL, model TEXT NOT NULL DEFAULT '',
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.8 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.8 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/student/:id/hunt', async(req,res)=>{
  ...
  const roster=(await pool.query('SELECT id FROM students WHERE class_id=$1 ORDER BY joined_at,id',[s.class_id])).rows; const rosterIndex=Math.max(0,roster.findIndex(x=>Number(x.id)===Number(s.id)));
  const tractorCount=Math.min(HUNT_MAX_TRACTORS,Math.max(1,Number(s.hunt_tractor_count)||3)); const team=huntTeamInfo(rosterIndex,tractorCount);
  ...
  const tractorName=n=>`Tractor ${n}${models.get(n)?' — '+models.get(n):''}`;
  const rotation=team.rotation.map(n=>tractorName(n)).join(' → ');
  const startingRole=huntRoleForStation(team.memberSlot,1);
  const groups=[]; for(let n=1;n<=tractorCount;n++){ const rows=q.rows.filter(x=>Number(x.tractor_no)===n); if(!rows.length) continue; const html=rows.map((x,i)=>{ const global=q.rows.findIndex(r=>r.id===x.id)+1; return `<tr><td>Station ${global}</td><td><b>${esc(x.item_label||x.station_name)}</b></td><td>${completed.has(x.id)?'<span class="pill results">Complete</span>':'<span class="pill">Not Scanned</span>'}</td></tr>`; }).join(''); groups.push(`<div class="card"><div class="eyebrow">TRACTOR ${n}</div><div class="big">${esc(models.get(n)||`Tractor ${n}`)}</div><table><tr><th>#</th><th>Hunt Item</th><th>Status</th></tr>${html}</table></div>`); }
  const unassigned=q.rows.filter(x=>!x.tractor_no); const extra=unassigned.length?`<div class="card"><div class="big">Additional Stations</div><table>${unassigned.map(x=>`<tr><td><b>${esc(x.station_name)}</b></td><td>${completed.has(x.id)?'Complete':'Not Scanned'}</td></tr>`).join('')}</table></div>`:'';
  const empty=q.rowCount?``:`<div class="card"><div class="alert"><b>Waiting for Instructor</b><br>Your instructor has not selected the scavenger-hunt items yet.</div></div>`;
  res.send(layout('Scavenger Hunt', `<div class="card"><div class="eyebrow">Hands-On Team Activity · v5.8</div><div class="big">Custom Tractor Technician Scavenger Hunt</div><p>Your instructor selected the tractor models and exact items to hunt for.</p><div class="grid"><div class="stat"><span>Your Team</span><b>${team.name}</b></div><div class="stat"><span>Start At</span><b>${esc(tractorName(team.rotation[0]))}</b></div><div class="stat"><span>Your First Role</span><b>${startingRole.name}</b></div><div class="stat"><span>Progress</span><b>${completed.size} / ${q.rowCount}</b><div class="progressbar"><span style="width:${q.rowCount?Math.round(completed.size/q.rowCount*100):0}%"></span></div></div></div><div class="alert"><b>Your tractor rotation:</b> ${esc(rotation)}<br><span class="small">Your role changes automatically at every station.</span></div><p><b>Roles:</b> Navigator · Locator · Tool Operator · Recorder · Verifier</p></div>${empty}${groups.join('')}${extra}<div class="card"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`));
});
...
app.get('/hunt-station/:cid/:sid', async(req,res)=>{
  ...
  const roster=(await pool.query('SELECT id FROM students WHERE class_id=$1 ORDER BY joined_at,id',[station.class_id])).rows; const rosterIndex=Math.max(0,roster.findIndex(x=>Number(x.id)===Number(student.id))); const team=huntTeamInfo(rosterIndex,station.hunt_tractor_count);
...
app.post('/hunt-station/:cid/:sid', async(req,res)=>{
  ...
  const answer=(req.body.answer||'').trim(),a=answer.toLowerCase(),e=(station.expected||'').toLowerCase(); const correct=!!a&&(!e||a.includes(e)||e.includes(a));
  ...
  if(total&&Number(p.done)>=total){ await pool.query("DELETE FROM results WHERE student_id=$1 AND activity='Scavenger Hunt'",[student.id]); await pool.query(`INSERT INTO results(student_id,activity,score,details) VALUES($1,'Scavenger Hunt',$2,$3)`,[student.id,score,JSON.stringify({completed:Number(p.done),total})]); }
  res.send(layout('Station Complete', `<div class="card center"><div class="big">Station Complete</div><p>${esc(station.item_label||station.station_name)}</p><div class="success">Answer recorded.</div><p>${p.done} of ${total} stations complete.</p><a class="btn" href="/student/${student.id}/hunt?token=${encodeURIComponent(station.join_token)}">View Hunt Progress</a></div>`));
});
...
app.get('/student/:id/scenario/:attemptId', async(req,res)=>{
  ...
  const done=new Set(actions.map(x=>x.action_key)); const totals=await recalcSimulation(a.id); const evidence=actions.length?actions.map((x,i)=>`<div class="sim-log ${x.penalty?'penalty':x.points?'good':''}"><b>${i+1}. ${esc(x.label)}</b> ${x.points?`<span class="sim-badge">+${x.points}</span>`:''}${x.penalty?`<span class="sim-badge">-${x.penalty}</span>`:''}<div style="margin-top:6px">${esc(x.result_text)}</div></div>`).join(''):'<p class="muted">No diagnostic evidence collected yet. Choose a tool or test.</p>';
  ...
  const tools=cats.map(cat=>{const list=sim.actions.filter(x=>x.category===cat); if(!list.length)return ''; return `<div class="sim-tool"><h3>${esc(cat)}</h3>${list.map(x=>done.has(x.key)?`<button type="button" disabled>✓ ${esc(x.label)}</button>`:`<form method="post" action="/student/${s.id}/scenario/${a.id}/action?token=${encodeURIComponent(s.join_token)}"><input type="hidden" name="action_key" value="${esc(x.key)}"><button class="${x.penalty?'alt':''}">${esc(x.label)}</button></form>`).join('')}</div>`}).join('');
...
app.get('/instructor/simulations',auth,async(req,res)=>{
  ...
  const currentClass=classes.find(c=>c.active)||classes[0]||null;
  const assignments=currentClass ? (await pool.query(`SELECT * FROM simulation_assignments WHERE class_id=$1 AND enabled=true ORDER BY configured_at DESC`,[currentClass.id])).rows : [];
  ...
  const sims=Object.values(DIAGNOSTIC_SIMULATIONS).map(sim=>{
    ...
    return `<div class="card" style="border-top:5px solid var(--red)">
      ...
      ${!currentClass?'<div class="alert"><b>No current class.</b> Start a class before releasing a simulation.</div>':`<form method="post" action="/instructor/simulations/${encodeURIComponent(sim.key)}/configure?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="margin-top:14px">
        ...
        <button>${selectedRoot&&selectedTractor?'Update Simulation':'Release This Simulation'}</button>
      </form>`}
      ${currentClass&&selectedRoot&&selectedTractor?`<div class="success" style="margin-top:10px"><b>Instructor Only:</b> Tractor: ${esc(selectedTractor)} · Root Cause: ${esc(rootCauseByKey(sim,selectedRoot)?.diagnosis||'')}</div>`:''}
...
app.post('/instructor/simulations/:key/configure',auth,async(req,res)=>{
  ...
  const c=(await pool.query(`SELECT * FROM classes ORDER BY active DESC,created_at DESC LIMIT 1`)).rows[0]; if(!c) return res.status(400).send('Start a class before releasing a simulation.');
  const classId=Number(c.id), rootKey=(req.body.root_cause_key||'').trim(), tractorModel=(req.body.tractor_model||'').trim(); if(!rootCauseByKey(sim,rootKey)||!validTractorForSim(sim,tractorModel)) return res.status(400).send('Choose a valid tractor and root cause.');
  ...
  await pool.query(`INSERT INTO simulation_assignments(class_id,simulation_key,root_cause_key,tractor_model,enabled,configured_at) VALUES($1,$2,$3,$4,true,now()) ON CONFLICT(class_id,simulation_key) DO UPDATE SET root_cause_key=$3,tractor_model=$4,enabled=true,configured_at=now()`,[classId,sim.key,rootKey,tractorModel]);
...
app.get('/instructor/builder/:id',auth,async(req,res)=>{
  const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c) return res.status(404).send('Class not found');
  const qs=await pool.query('SELECT * FROM quiz_questions WHERE class_id=$1 ORDER BY id',[c.id]); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY COALESCE(tractor_no,99),id',[c.id]);
  ...
  const hcards=hs.rows.map(h=>`<div class="q"><span class="pill">Tractor ${h.tractor_no||'—'}</span> <b>${esc(h.tractor_model||models.get(Number(h.tractor_no))||'')} — ${esc(h.item_label||h.station_name)}</b><div class="small muted">${esc(h.task)}</div><div class="small"><b>Expected:</b> ${esc(h.expected)}</div><form method="post" action="/instructor/builder/${c.id}/station/${h.id}/delete" style="margin-top:8px"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="danger" onclick="return confirm('Delete this hunt item?')">Delete Hunt Item</button></form></div>`).join('');
  const tractorCount=Math.min(HUNT_MAX_TRACTORS,Math.max(1,Number(c.hunt_tractor_count)||3));
  const modelOptions=HUNT_MODEL_SUGGESTIONS.map(x=>`<option value="${esc(x)}"></option>`).join('');
  const itemChecks=HUNT_ITEM_LIBRARY.map(x=>`<label style="font-weight:500"><input type="checkbox" name="items" value="${esc(x.id)}" style="width:auto;margin-right:8px">${esc(x.label)}</label>`).join('');
  const tractorCards=Array.from({length:tractorCount},(_,i)=>i+1).map(n=>`<div class="card"><div class="eyebrow">TRACTOR ${n}</div><div class="big">${esc(models.get(n)||'Choose Model')}</div><form method="post" action="/instructor/builder/${c.id}/hunt/tractor/${n}/model"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Tractor Model<input list="hunt-models" name="model" value="${esc(models.get(n)||'')}" placeholder="Example: Fifty One Hundred Refresh" required></label><button>Save Tractor ${n} Model</button></form><hr style="border:0;border-top:1px solid #eee;margin:18px 0"><form method="post" action="/instructor/builder/${c.id}/hunt/tractor/${n}/items"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><div class="big" style="font-size:19px">Choose Hunt Items</div><div style="max-height:390px;overflow:auto;border:1px solid #eee;border-radius:10px;padding:10px;margin:10px 0">${itemChecks}</div><button>Add Selected Items to Tractor ${n}</button></form></div>`).join('');
  const tractorSelectOptions=Array.from({length:tractorCount},(_,i)=>`<option value="${i+1}">Tractor ${i+1}${models.get(i+1)?' — '+esc(models.get(i+1)):''}</option>`).join('');
  res.send(layout('Training Builder', `<datalist id="hunt-models">${modelOptions}</datalist><div class="toolbar"><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Back to Class</a><a class="btn" target="_blank" href="/instructor/class/${c.id}/hunt-qr?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Print Hunt QR Codes</a></div><div class="grid"><div class="card"><div class="big">Add Quiz Question</div><form method="post" action="/instructor/builder/${c.id}/question"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Topic / Module<input name="topic" placeholder="Example: CAN / J1939"></label><label>Question<textarea name="question" required></textarea></label>${['A','B','C','D'].map((x,i)=>`<label>${x}<input name="c${i}" required></label>`).join('')}<label>Correct Answer<select name="answer"><option value="0">A</option><option value="1">B</option><option value="2">C</option><option value="3">D</option></select></label><label>Explanation / Student Feedback<textarea name="explanation" rows="3"></textarea></label><button>Add Question</button></form><p>${qs.rowCount} question(s) saved.</p>${qcards}</div><div class="card"><div class="eyebrow">SCAVENGER HUNT · v5.8</div><div class="big">Build Your Scavenger Hunt</div><p>Choose how many tractors you will use, set the model for each tractor, then choose exactly what the students hunt for.</p><form method="post" action="/instructor/builder/${c.id}/hunt/tractor-count"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>How Many Tractors Will You Use?<select name="tractor_count">${[1,2,3,4,5].map(n=>`<option value="${n}" ${n===tractorCount?'selected':''}>${n} Tractor${n===1?'':'s'}</option>`).join('')}</select></label><button>Save Number of Tractors</button></form><div class="alert" style="margin-top:12px"><b>Automatic:</b> ${tractorCount} tractor${tractorCount===1?'':'s'} = ${tractorCount} team${tractorCount===1?'':'s'}. Team rotation and technician roles are assigned automatically.</div><p class="small"><b>Roles:</b> Navigator → Locator → Tool Operator → Recorder → Verifier. Roles rotate at every station.</p><form method="post" action="/instructor/builder/${c.id}/hunt/clear"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><button class="danger" onclick="return confirm('Clear all scavenger-hunt stations and hunt progress for this class?')">Clear Hunt Stations</button></form></div></div><div class="grid">${tractorCards}</div><div class="card"><div class="big">Add a Custom Hunt Item</div><p class="small muted">Use this for any component, new model change, measurement, service point, or inspection item not listed above.</p><form method="post" action="/instructor/builder/${c.id}/station"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Tractor<select name="tractor_no" required>${tractorSelectOptions}</select></label><label>Item Name<input name="name" required placeholder="Example: New battery location"></label><label>Technician Task<textarea name="task" required></textarea></label><label>Expected Answer / Verification<input name="expected" required></label><button>Add Custom Hunt Item</button></form></div><div class="card"><div class="section-title"><div class="big">Current Hunt</div><span class="pill">${hs.rowCount} items</span></div>${hcards||'<p>No hunt items yet. Set tractor models and select hunt items above.</p>'}</div>`));
});
...
app.post('/instructor/builder/:id/hunt/tractor-count',auth,async(req,res)=>{ const cid=Number(req.params.id),count=Math.min(HUNT_MAX_TRACTORS,Math.max(1,Number(req.body.tractor_count)||3)); const old=(await pool.query('SELECT hunt_tractor_count FROM classes WHERE id=$1',[cid])).rows[0]; await pool.query('UPDATE classes SET hunt_tractor_count=$2 WHERE id=$1',[cid,count]); if(old&&Number(old.hunt_tractor_count)!==count){ const removed=(await pool.query('SELECT id FROM hunt_stations WHERE class_id=$1 AND COALESCE(tractor_no,999)>$2',[cid,count])).rows.map(x=>x.id); if(removed.length) await pool.query('DELETE FROM hunt_stations WHERE id=ANY($1::int[])',[removed]); await pool.query('DELETE FROM hunt_tractors WHERE class_id=$1 AND tractor_no>$2',[cid,count]); await pool.query(`DELETE FROM results WHERE student_id IN (SELECT id FROM students WHERE class_id=$1) AND activity='Scavenger Hunt'`,[cid]); } res.redirect(`/instructor/builder/${cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
app.post('/instructor/builder/:id/hunt/tractor/:slot/model',auth,async(req,res)=>{ const cid=Number(req.params.id),cfg=(await pool.query('SELECT hunt_tractor_count FROM classes WHERE id=$1',[cid])).rows[0],max=Math.min(HUNT_MAX_TRACTORS,Math.max(1,Number(cfg?.hunt_tractor_count)||3)),slot=Math.min(max,Math.max(1,Number(req.params.slot)||1)),model=(req.body.model||'').trim(); await pool.query(`INSERT INTO hunt_tractors(class_id,tractor_no,model) VALUES($1,$2,$3) ON CONFLICT(class_id,tractor_no) DO UPDATE SET model=$3`,[cid,slot,model]); await pool.query('UPDATE hunt_stations SET tractor_model=$3,station_name=CASE WHEN item_label<>\'\' THEN $3 || \' — \' || item_label ELSE station_name END WHERE class_id=$1 AND tractor_no=$2',[cid,slot,model]); res.redirect(`/instructor/builder/${cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
app.post('/instructor/builder/:id/hunt/tractor/:slot/items',auth,async(req,res)=>{ const cid=Number(req.params.id),cfg=(await pool.query('SELECT hunt_tractor_count FROM classes WHERE id=$1',[cid])).rows[0],max=Math.min(HUNT_MAX_TRACTORS,Math.max(1,Number(cfg?.hunt_tractor_count)||3)),slot=Math.min(max,Math.max(1,Number(req.params.slot)||1)); const mq=(await pool.query('SELECT model FROM hunt_tractors WHERE class_id=$1 AND tractor_no=$2',[cid,slot])).rows[0],model=mq?.model||`Tractor ${slot}`; let ids=req.body.items||[]; if(!Array.isArray(ids)) ids=[ids]; for(const id of ids){ const item=HUNT_ITEM_LIBRARY.find(x=>x.id===id); if(!item) continue; const exists=await pool.query('SELECT 1 FROM hunt_stations WHERE class_id=$1 AND tractor_no=$2 AND item_label=$3',[cid,slot,item.label]); if(exists.rowCount) continue; await pool.query('INSERT INTO hunt_stations(class_id,station_name,task,expected,tractor_no,tractor_model,item_label) VALUES($1,$2,$3,$4,$5,$6,$7)',[cid,`${model} — ${item.label}`,item.task,item.expected,slot,model,item.label]); } res.redirect(`/instructor/builder/${cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
app.post('/instructor/builder/:id/hunt/clear',auth,async(req,res)=>{ const cid=Number(req.params.id),ids=(await pool.query('SELECT id FROM hunt_stations WHERE class_id=$1',[cid])).rows.map(x=>x.id); if(ids.length) await pool.query('DELETE FROM hunt_progress WHERE station_id=ANY($1::int[])',[ids]); await pool.query(`DELETE FROM results WHERE student_id IN (SELECT id FROM students WHERE class_id=$1) AND activity='Scavenger Hunt'`,[cid]); await pool.query('DELETE FROM hunt_stations WHERE class_id=$1',[cid]); res.redirect(`/instructor/builder/${cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
app.post('/instructor/builder/:id/station',auth,async(req,res)=>{ const cid=Number(req.params.id),cfg=(await pool.query('SELECT hunt_tractor_count FROM classes WHERE id=$1',[cid])).rows[0],max=Math.min(HUNT_MAX_TRACTORS,Math.max(1,Number(cfg?.hunt_tractor_count)||3)),slot=Math.min(max,Math.max(1,Number(req.body.tractor_no)||1)); const mq=(await pool.query('SELECT model FROM hunt_tractors WHERE class_id=$1 AND tractor_no=$2',[cid,slot])).rows[0],model=mq?.model||`Tractor ${slot}`; await pool.query('INSERT INTO hunt_stations(class_id,station_name,task,expected,tractor_no,tractor_model,item_label) VALUES($1,$2,$3,$4,$5,$6,$7)',[cid,`${model} — ${req.body.name}`,req.body.task,req.body.expected,slot,model,req.body.name]); res.redirect(`/instructor/builder/${cid}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}`); });
