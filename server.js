...
app.post('/student/:id/quiz', async(req,res)=>{
  ...
  res.send(layout('Quiz Complete', `<div class="card center"><div class="eyebrow">Training Hub v3.9.2</div><div class="big">Quiz Complete</div><div class="code">${score}%</div><p>${correct} of ${q.rowCount} correct</p><p class="muted">Questions missed: <b>${missed.length}</b></p></div>${certHtml}${reviewHtml}<div class="card center"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`));
});
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=renderQuizReview({review:repaired.review,missed:repaired.missed},{score:result.score,showAll:true});
  res.send(layout('Quiz Review', `<div class="card" style="border-top:5px solid var(--red)"><div class="eyebrow">Training Hub v3.9.2</div><div class="big">Quiz Review — Correct Answers</div><p>Your latest quiz score: <b>${result.score}%</b></p>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/instructor/student/:id',auth,async(req,res)=>{
  ...
  const quizResult=[...results.rows].reverse().find(x=>x.activity==='Module Quiz'); const reviewInfo=await repairQuizReviewData(s.id,quizResult?.details||{}); const missed=reviewInfo.missed; const quizReview=missed.length?missed.map((m,i)=>`<div class="q feedback-miss"><b>Missed ${i+1}${m.topic?' · '+esc(m.topic):''}</b><p><b>Question:</b> ${esc(m.question||'')}</p><p><b>Student Answer:</b> ${esc(m.selected||'No answer')}</p><p><b>Correct Answer:</b> ${esc(m.correct||'')}</p><p><b>Explanation:</b> ${esc(m.explanation||'Review this topic with your instructor.')}</p></div>`).join(''):'<div class="success">No missed quiz questions recorded.</div>';
...
async function repairQuizReviewData(studentId,details={}){
  let info=quizReviewData(details);
  const hasUseful=info.review.some(x=>x && x.question && x.correct);
  if(hasUseful) return info;
  ...
  if(!q.rowCount) return info;
  const review=q.rows.map(x=>({question:x.question||'',topic:x.topic||'',selected:Number.isInteger(x.selected_answer)?(x.choices?.[x.selected_answer]||'No answer'):'Answer not recorded in older attempt',correct:x.choices?.[x.answer_index]||'',explanation:x.explanation||'Review this topic with your instructor.',is_correct:!!x.is_correct}));
  return {review,missed:review.filter(x=>!x.is_correct)};
}
