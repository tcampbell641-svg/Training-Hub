...
app.get('/instructor/hunt/:id',auth,async(req,res)=>{
 ...
 const tractorCards=Array.from({length:count},(_,i)=>i+1).map(n=>{
   ...
   const visibleLibrary=model ? HUNT_ITEM_LIBRARY.filter(x=>!x.id.startsWith('refresh_') || is5100RefreshModel(model)) : [];
