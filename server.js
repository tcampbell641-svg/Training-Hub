...
app.get('/instructor/prepost',auth,async(req,res)=>{
  ...
  res.send(layout('Pre/Post Improvement',`<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Command Center</a></div><div class="card" style="background:linear-gradient(135deg,#171717,#5a0b10);color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">KNOWLEDGE IMPROVEMENT · v5.6</div><div class="big" style="font-size:30px">Pre-Test vs Post-Test</div><p style="margin-bottom:0">First completed quiz = Pre-Test. Most recent later quiz = Post-Test.</p></div><div class="grid">${cards||'<div class="card">No classes yet.</div>'}</div>`));
});
