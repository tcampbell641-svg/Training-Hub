...
const LABEL_SPECS={
  '30':{label:'30 mm Compact',w:354,h:472,qr:270,title:24,small:17,task:false},
  '40x60':{label:'40 × 60 mm',w:472,h:709,qr:342,title:27,small:18,task:true},
  '50x80':{label:'50 × 80 mm',w:591,h:945,qr:425,title:32,small:20,task:true},
  '50':{label:'Standard T50M / 50 mm',w:591,h:709,qr:390,title:30,small:19,task:true}
};
function wrapWords(text,maxChars=32,maxLines=3){ const words=String(text||'').trim().split(/\s+/).filter(Boolean),lines=[]; let line=''; for(const word of words){const next=line?line+' '+word:word;if(next.length<=maxChars)line=next;else{if(line)lines.push(line);line=word;if(lines.length>=maxLines-1)break;}} if(line&&lines.length<maxLines)lines.push(line); return lines; }
async function makeHuntLabelPng({station,index,classRow,url,size}){
  const spec=LABEL_SPECS[size]||LABEL_SPECS['50x80'];
  const qrData=await QRCode.toDataURL(url,{width:spec.qr,margin:4,color:{dark:'#000000',light:'#FFFFFF'}});
  const item=String(station.item_label||station.station_name||`Station ${index+1}`).replace(/^.*?—\s*/,'').trim();
  const titleLines=wrapWords(item,size==='30'?18:28,size==='30'?2:3);
  const taskLines=spec.task?wrapWords(station.task||'',size==='50x80'?48:38,3):[];
  const tractor=station.tractor_model?`Tractor ${station.tractor_no} · ${station.tractor_model}`:`Tractor ${station.tractor_no||''}`;
  const pad=Math.round(spec.w*0.055),top=pad,lineH=Math.round(spec.title*1.08); let y=top+spec.small;
  const titleSvg=titleLines.map((ln,i)=>`<text x="${spec.w/2}" y="${y+28+i*lineH}" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${spec.title}" font-weight="700" fill="#000">${xmlEsc(ln)}</text>`).join('');
  const qrY=y+34+titleLines.length*lineH+8,qrX=Math.round((spec.w-spec.qr)/2),scanY=qrY+spec.qr+Math.round(spec.small*1.2);
  const taskSvg=taskLines.map((ln,i)=>`<text x="${pad}" y="${scanY+36+i*Math.round(spec.small*1.2)}" font-family="Arial,Helvetica,sans-serif" font-size="${spec.small}" fill="#000">${xmlEsc(ln)}</text>`).join('');
  const classY=spec.h-pad-10;
  const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${spec.w}" height="${spec.h}" viewBox="0 0 ${spec.w} ${spec.h}"><rect width="100%" height="100%" fill="#fff"/><rect x="4" y="4" width="${spec.w-8}" height="${spec.h-8}" fill="none" stroke="#000" stroke-width="3"/><text x="${pad}" y="${top+spec.small}" font-family="Arial,Helvetica,sans-serif" font-size="${spec.small}" font-weight="700" fill="#000">STATION ${index+1}</text><text x="${spec.w-pad}" y="${top+spec.small}" text-anchor="end" font-family="Arial,Helvetica,sans-serif" font-size="${spec.small}" font-weight="700" fill="#000">${xmlEsc(tractor)}</text>${titleSvg}<image href="${qrData}" x="${qrX}" y="${qrY}" width="${spec.qr}" height="${spec.qr}"/><text x="${spec.w/2}" y="${scanY}" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${Math.round(spec.small*1.15)}" font-weight="700" letter-spacing="2" fill="#000">SCAN QR</text>${taskSvg}<text x="${spec.w/2}" y="${classY}" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${Math.max(13,spec.small-3)}" font-weight="700" fill="#000">Class ${xmlEsc(classRow.code)} · ${xmlEsc(classRow.course)}</text></svg>`;
...
app.get('/instructor/class/:id/hunt-label-print',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found');
  res.send(layout('Generate T50M Labels', `<div class="toolbar"><a class="btn light" href="/instructor/hunt/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Back to Scavenger Hunt</a><a class="btn light" href="/instructor/class/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Class Dashboard</a></div><div class="card" style="max-width:720px;margin:0 auto"><div class="eyebrow">SCAVENGER HUNT</div><div class="big">Generate T50M Label Files</div><p>Choose the label size loaded in your T50M Pro. The Training Hub will generate actual PNG label files you can import into SUPVAN Editor. This does not depend on Chrome printing.</p><form method="get" action="/instructor/class/${c.id}/hunt-label-files"><input type="hidden" name="pin" value="${esc(INSTRUCTOR_PIN)}"><label>Label Size<select name="size" required><option value="" selected disabled>— Select Label Size —</option><option value="30">30 mm Compact</option><option value="40x60">40 × 60 mm</option><option value="50x80">50 × 80 mm</option><option value="50">Standard T50M / 50 mm</option></select></label><button type="submit" style="width:100%;margin-top:12px">Generate Label Files</button></form><div class="alert" style="margin-top:14px"><b>SUPVAN Editor workflow:</b> Generate the files here, download the PNG labels, import them into SUPVAN Editor, set the same physical label size, and print from SUPVAN Editor.</div></div>`));
});

app.get('/instructor/class/:id/hunt-label-files',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const size=String(req.query.size||''); if(!LABEL_SPECS[size])return res.status(400).send('Choose a valid label size.'); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY COALESCE(tractor_no,99),id',[c.id]); const count=Math.min(5,Math.max(1,Number(c.hunt_tractor_count)||3)),target=Math.min(12,Math.max(1,Number(c.hunt_items_per_tractor)||5)); const counts=Array.from({length:count},(_,i)=>hs.rows.filter(x=>Number(x.tractor_no)===i+1).length); if(!counts.every(n=>n===target))return res.send(layout('Hunt Not Balanced',`<div class="card"><div class="big">Finish the scavenger hunt setup first</div><div class="alert">Every tractor must have exactly ${target} hunt items before label files can be generated.</div><a class="btn" href="/instructor/hunt/${c.id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Back to Scavenger Hunt</a></div>`)); const pin=encodeURIComponent(INSTRUCTOR_PIN); const cards=hs.rows.map((h,i)=>{const item=esc((h.item_label||h.station_name||`Station ${i+1}`).replace(/^.*?—\s*/,'')); return `<div class="card"><div class="eyebrow">STATION ${i+1}</div><div class="big" style="font-size:20px">${item}</div><p class="muted">${esc(h.tractor_model||'Tractor')} · Tractor ${esc(h.tractor_no||'')}</p><a class="btn" href="/instructor/class/${c.id}/hunt-label-png/${h.id}?size=${encodeURIComponent(size)}&pin=${pin}">Download PNG</a></div>`}).join(''); res.send(layout('Generated T50M Labels', `<div class="toolbar"><a class="btn light" href="/instructor/class/${c.id}/hunt-label-print?pin=${pin}">← Change Label Size</a><a class="btn" href="/instructor/class/${c.id}/hunt-label-zip?size=${encodeURIComponent(size)}&pin=${pin}">Download All as ZIP</a></div><div class="card"><div class="big">${esc(LABEL_SPECS[size].label)} Label Files</div><p>${hs.rowCount} station label${hs.rowCount===1?'':'s'} ready. These are PNG image files sized for the selected label stock and designed for importing into SUPVAN Editor.</p></div><div class="grid">${cards}</div>`)); });

app.get('/instructor/class/:id/hunt-label-png/:sid',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const size=String(req.query.size||''); if(!LABEL_SPECS[size])return res.status(400).send('Invalid label size.'); const h=(await pool.query('SELECT * FROM hunt_stations WHERE id=$1 AND class_id=$2',[req.params.sid,c.id])).rows[0]; if(!h)return res.status(404).send('Station not found'); const rows=(await pool.query('SELECT id FROM hunt_stations WHERE class_id=$1 ORDER BY COALESCE(tractor_no,99),id',[c.id])).rows; const index=Math.max(0,rows.findIndex(x=>Number(x.id)===Number(h.id))); const origin=`${req.protocol}://${req.get('host')}`,url=`${origin}/hunt-station/${c.id}/${h.id}`; const png=await makeHuntLabelPng({station:h,index,classRow:c,url,size}); const item=safeFile(h.item_label||h.station_name||`station_${index+1}`); res.setHeader('Content-Type','image/png'); res.setHeader('Content-Disposition',`attachment; filename="station_${String(index+1).padStart(2,'0')}_${item}_${size}.png"`); res.send(png); });

app.get('/instructor/class/:id/hunt-label-zip',auth,async(req,res)=>{ const c=(await pool.query('SELECT * FROM classes WHERE id=$1',[req.params.id])).rows[0]; if(!c)return res.status(404).send('Class not found'); const size=String(req.query.size||''); if(!LABEL_SPECS[size])return res.status(400).send('Invalid label size.'); const hs=await pool.query('SELECT * FROM hunt_stations WHERE class_id=$1 ORDER BY COALESCE(tractor_no,99),id',[c.id]); if(!hs.rowCount)return res.status(404).send('No scavenger hunt stations found.'); res.setHeader('Content-Type','application/zip'); res.setHeader('Content-Disposition',`attachment; filename="class_${safeFile(c.code)}_T50M_${safeFile(size)}_labels.zip"`); const archive=archiver('zip',{zlib:{level:9}}); archive.on('error',err=>{console.error(err); if(!res.headersSent)res.status(500).send('Could not build label ZIP.'); else res.end();}); archive.pipe(res); const origin=`${req.protocol}://${req.get('host')}`; for(let i=0;i<hs.rows.length;i++){const h=hs.rows[i],url=`${origin}/hunt-station/${c.id}/${h.id}`; const png=await makeHuntLabelPng({station:h,index:i,classRow:c,url,size}); const item=safeFile(h.item_label||h.station_name||`station_${i+1}`); archive.append(png,{name:`station_${String(i+1).padStart(2,'0')}_${item}_${size}.png`}); } archive.append(`Mahindra Training Hub T50M Labels\nClass: ${c.code}\nCourse: ${c.course}\nLabel size: ${LABEL_SPECS[size].label}\n\nImport each PNG into SUPVAN Editor and set the physical label stock to the same size before printing.\n`,{name:'README.txt'}); await archive.finalize(); });
