...
const DIAGNOSTIC_SIMULATIONS = {
  '5100-crank-no-start': {
    key:'5100-crank-no-start',
    title:'Fifty One Hundred Refresh — Cranks, No Start',
    model:'Mahindra Fifty One Hundred Refresh',
    workOrder:'WO-5100-001',
    complaint:'Engine cranks normally but will not start. No smoke is seen from the exhaust.',
    hours:'412',
    assignment:'Diagnose the root cause, select the correct repair, and verify the repair.',
    rootCauses:[
      {key:'cam_sensor',diagnosis:'Failed camshaft position sensor',repair:'Replace camshaft position sensor and verify synchronization',points:{cam_circuit:20,cam_meter:15},overrides:{live_data:'Engine RPM: 215 RPM | Rail pressure: 4.2 MPa | Cam synchronization: NO | Battery voltage: 10.9 V | Engine state: 33',cam_meter:'Supply: 4.98 V (spec 5.0 V) | Ground voltage drop: 0.03 V | Signal: no switching signal while cranking.',fuel_filter:'Fuel supply is present and the filter is not restricted. No evidence here proves a fuel-filter failure.',injector_resistance:'Injector electrical checks are consistent across all cylinders.'}},
      {key:'ecu',diagnosis:'Failed engine ECU',repair:'Replace ECU, program as required, clear faults, and verify engine starts',points:{wiring:20,ecu_power_ground:15},overrides:{live_data:'Engine RPM: 214 RPM | Rail pressure: 27.8 MPa | Cam synchronization: YES | Battery voltage: 11.0 V | Injector command: NO',cam_meter:'Cam sensor supply, ground, and switching signal are all within specification.',ecu_power_ground:'ECU battery feed: 12.1 V | Ignition feed: 11.9 V | ECU ground voltage drop while cranking: 0.04 V | Inputs are valid but injector command remains absent.',fuel_filter:'Fuel supply and filter restriction are within specification.',injector_resistance:'Injector electrical checks are consistent across all cylinders.'}},
      {key:'fuel_filter',diagnosis:'Restricted fuel filter',repair:'Replace fuel filter, prime fuel system, and verify starting and rail pressure',points:{fuel_filter:20,rail_pressure:15},overrides:{live_data:'Engine RPM: 218 RPM | Rail pressure: 6.1 MPa | Cam synchronization: YES | Battery voltage: 10.9 V | Engine state: 33',rail_pressure:'Rail pressure rises only to 6.1 MPa while cranking. Low-pressure supply restriction is indicated.',fuel_filter:'Fuel-filter inlet vacuum is excessive and flow after the filter is restricted. Restriction is proven.',cam_meter:'Cam sensor power, ground, and signal are within specification.',injector_resistance:'Injector electrical checks are consistent across all cylinders.'}},
      {key:'hp_pump',diagnosis:'Failed high-pressure fuel pump',repair:'Replace high-pressure fuel pump, perform required fuel-system service, and verify rail pressure and starting',points:{rail_pressure:20,pump_control:15},overrides:{live_data:'Engine RPM: 216 RPM | Rail pressure: 5.4 MPa | Cam synchronization: YES | Battery voltage: 11.0 V | Fuel metering command: HIGH',rail_pressure:'Rail pressure remains at 5.4 MPa while cranking even though low-pressure supply is within specification.',fuel_filter:'Fuel supply volume and filter restriction are within specification.',pump_control:'Fuel metering command and circuit are within specification, but commanded rail pressure is not achieved.',cam_meter:'Cam sensor power, ground, and signal are within specification.'}},
      {key:'injectors',diagnosis:'Injector electrical failure',repair:'Repair the failed injector circuit/component and verify injector command and engine starting',points:{injector_resistance:20,injector_pulse:15},overrides:{live_data:'Engine RPM: 217 RPM | Rail pressure: 29.2 MPa | Cam synchronization: YES | Battery voltage: 11.0 V | Injector command present',injector_resistance:'One injector circuit measures open compared with the remaining injectors. The failure is repeatable.',injector_pulse:'Injector command is present at the ECU. One injector circuit does not carry current during cranking.',fuel_filter:'Fuel supply and filter restriction are within specification.',cam_meter:'Cam sensor power, ground, and signal are within specification.'}}
    ],
    actions:[
...
function simByKey(key){ return DIAGNOSTIC_SIMULATIONS[key]||DIAGNOSTIC_SIMULATIONS['5100-crank-no-start']; }
function rootCauseByKey(sim,key){ return sim.rootCauses.find(x=>x.key===key)||null; }
function simForRoot(key,rootKey){
  const base=simByKey(key), root=rootCauseByKey(base,rootKey); if(!root) return null;
  return {...base,correctDiagnosis:root.diagnosis,correctRepair:root.repair,rootCauseKey:root.key,actions:base.actions.map(a=>({...a,points:(a.key==='verify_complaint'||a.key==='fault_codes'||a.key==='live_data')?(a.points||0):(root.points[a.key]||0),result:(root.overrides[a.key]||a.result)}))};
}
...
app.get('/student/:id', async(req,res)=>{
  ...
  const simAssignment=await getSimulationAssignment(s.class_id,'5100-crank-no-start');
  ...
  res.send(layout('Technician Home', `<div style="background:linear-gradient(135deg,#171717,#3a080b);color:white;border-radius:18px;padding:24px;margin-bottom:18px;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#f0b8bb">Technician Training Portal · v5.2</div><div class="big" style="font-size:30px">Welcome, ${esc(s.name)}</div><p style="margin-bottom:0">${esc(s.course)} · ${esc(s.dealer)} · Class ${esc(s.code)}</p></div>${certificateCard}<div class="grid"><div class="card"><div class="big">Module Quiz</div><p>Knowledge test. Your instructor can watch your progress while you work.</p>${done.has('Module Quiz')?`<div class="success">Latest Score · ${quizScore}%</div><div class="toolbar" style="margin-top:10px"><a class="btn light" href="/student/${s.id}/quiz-review?token=${encodeURIComponent(s.join_token)}">Review Latest Answers</a><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a></div>`:`<a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Start Quiz</a>`}</div><div class="card"><div class="big">QR Scavenger Hunt</div><p>Scan the QR code posted at each training station.</p>${done.has('Scavenger Hunt')?'<div class="success">Completed</div>':`<a class="btn" href="/student/${s.id}/hunt?token=${encodeURIComponent(s.join_token)}">View Hunt Progress</a>`}</div><div class="card"><div class="big">Diagnostic Failure Simulation</div><p>Enter the virtual shop bay and diagnose a crank/no-start failure step by step.</p>${!simAssignment?`<div class="alert"><b>Waiting for Instructor</b><br>The instructor must select the planted root cause before this simulation can begin.</div>`:done.has('Failure Simulation')?`<div class="success">Latest simulation completed</div><a class="btn" style="margin-top:10px" href="/student/${s.id}/scenario?token=${encodeURIComponent(s.join_token)}">Review / Retake Simulation</a>`:`<a class="btn" href="/student/${s.id}/scenario?token=${encodeURIComponent(s.join_token)}">Enter Diagnostic Bay</a>`}</div><div class="card"><div class="big">Training Feedback</div><p>Tell us what helped and what should be improved.</p>${feedback?'<div class="success">Feedback Submitted — Thank You</div>':allCore?`<a class="btn" href="/student/${s.id}/feedback?token=${encodeURIComponent(s.join_token)}">Give Training Feedback</a>`:'<div class="muted">Available after the training activities are finished.</div>'}</div></div>`));
});
...
app.get('/student/:id/quiz-review', async(req,res)=>{
  ...
  const reviewHtml=repaired.review.length?renderAttemptReview(repaired.review,old.score,true):`<div class="alert"><b>This older attempt has no recoverable question snapshot.</b> New v5.2 attempts are stored permanently.</div>`;
  res.send(layout('Quiz Review', `<div class="card"><div class="eyebrow">Legacy Quiz Record · v5.2 Recovery</div><div class="big">Quiz Review</div>${reviewHtml}<div class="toolbar" style="margin-top:16px"><a class="btn" href="/student/${s.id}/quiz?token=${encodeURIComponent(s.join_token)}">Retake Quiz</a><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div></div>`));
});
...
app.get('/student/:id/scenario', async(req,res)=>{
  const s=await studentContext(req.params.id,req.query.token); if(!s) return res.status(403).send('Invalid session');
  const latest=(await pool.query(`SELECT * FROM simulation_attempts WHERE student_id=$1 ORDER BY COALESCE(completed_at,started_at) DESC,id DESC LIMIT 1`,[s.id])).rows[0];
  if(latest?.status==='in_progress') return res.redirect(`/student/${s.id}/scenario/${latest.id}?token=${encodeURIComponent(s.join_token)}`);
  const previous=latest?.status==='completed'?`<div class="success"><b>Previous Simulation:</b> ${latest.score}% · Completed ${new Date(latest.completed_at).toLocaleString()}</div><div class="toolbar" style="margin-top:10px"><a class="btn light" href="/student/${s.id}/scenario/${latest.id}/review?token=${encodeURIComponent(s.join_token)}">Review Previous Attempt</a></div>`:'';
  const assignment=await getSimulationAssignment(s.class_id,'5100-crank-no-start');
  if(!assignment) return res.send(layout('Diagnostic Failure Simulation', `<div class="card"><div class="big">Waiting for Instructor</div><p>Your instructor must select the planted root cause before this simulation is released.</p><a class="btn light" href="/student/${s.id}?token=${encodeURIComponent(s.join_token)}">Back to Training</a></div>`));
  const sim=simForRoot('5100-crank-no-start',assignment.root_cause_key);
  res.send(layout('Diagnostic Failure Simulation', `<div class="card" style="border-top:6px solid var(--red)"><div class="eyebrow">Diagnostic Failure Simulation · Level 2</div><div class="big">${esc(sim.title)}</div><p>This is a virtual shop-bay diagnostic. You choose what to inspect, what data to retrieve, what tests to perform, and when you have enough proof to make the repair.</p>${previous}<div class="alert" style="margin-top:16px"><b>Customer Complaint:</b> ${esc(sim.complaint)}</div><p><b>Scoring:</b> 100 points possible. Correct diagnostic process earns points. Replacing unproven parts triggers a <b>15-point Parts Cannon penalty</b> for each part.</p><form method="post" action="/student/${s.id}/scenario/start?token=${encodeURIComponent(s.join_token)}"><button>Start Diagnostic Simulation</button></form></div>`));
});

app.post('/student/:id/scenario/start', async(req,res)=>{
  const s=await studentContext(req.params.id,req.query.token); if(!s) return res.status(403).send('Invalid session');
  const existing=(await pool.query(`SELECT * FROM simulation_attempts WHERE student_id=$1 AND status='in_progress' ORDER BY started_at DESC LIMIT 1`,[s.id])).rows[0];
  if(existing) return res.redirect(`/student/${s.id}/scenario/${existing.id}?token=${encodeURIComponent(s.join_token)}`);
  const assignment=await getSimulationAssignment(s.class_id,'5100-crank-no-start');
  if(!assignment) return res.status(400).send('Instructor has not selected a root cause for this simulation.');
  const sim=simForRoot('5100-crank-no-start',assignment.root_cause_key);
  const a=(await pool.query(`INSERT INTO simulation_attempts(student_id,class_id,simulation_key,title,work_order,root_cause_key,assigned_diagnosis,assigned_repair,status,started_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'in_progress',now()) RETURNING *`,[s.id,s.class_id,sim.key,sim.title,sim.workOrder,sim.rootCauseKey,sim.correctDiagnosis,sim.correctRepair])).rows[0];
...
app.post('/student/:id/scenario/:attemptId/action', async(req,res)=>{
  ...
  if(action.category==='Replace Part'){
    const replacementRules={
      replace_cam_sensor:{root:'cam_sensor',proof:['cam_circuit','cam_meter']},
      replace_ecu:{root:'ecu',proof:['wiring','ecu_power_ground']},
      replace_fuel_filter:{root:'fuel_filter',proof:['fuel_filter','rail_pressure']},
      replace_hp_pump:{root:'hp_pump',proof:['rail_pressure','pump_control']},
      replace_injectors:{root:'injectors',proof:['injector_resistance','injector_pulse']}
    };
    const rule=replacementRules[action.key];
    if(rule && rule.root===a.root_cause_key){
      ...
      const proven=rule.proof.every(k=>proofRows.includes(k));
      ...
      else { actionPenalty=15; actionResult='PARTS CANNON PENALTY: this happens to be the failed component, but it was replaced before the required proof-of-failure tests were completed.'; }
    }
  }
...
app.post('/student/:id/scenario/:attemptId/final', async(req,res)=>{
  ...
  const details={attempt_id:a.id,simulation_key:a.simulation_key,work_order:sim.workOrder,diagnosis,repair,diagnosis_correct:diagnosisCorrect,repair_correct:repairCorrect,positive_points:totals.positive,penalty_points:totals.penalty,actions:actions.map(x=>({action:x.label,result:x.result_text,points:x.points,penalty:x.penalty}))};
...
app.get('/instructor/dashboard',auth,async(req,res)=>{
  ...
  res.send(layout('Instructor Dashboard', `<div style="background:linear-gradient(135deg,#171717,#5a0b10);color:#fff;border-radius:22px;padding:28px;border-bottom:7px solid var(--red);box-shadow:0 8px 24px rgba(0,0,0,.12)"><div class="eyebrow" style="color:#ffb9bd">MAHINDRA TECHNICIAN TRAINING HUB · VERSION 5.2</div><h1 style="margin:7px 0 4px;font-size:36px">Instructor Command Center</h1><p style="margin:0;color:#eee">${esc(settings.home_message||'Start classes, build activities, watch technicians, and print training records.')}</p><div class="toolbar" style="margin-top:18px"><a class="btn" href="/instructor/new?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">+ Start New Class</a><a class="btn light" href="/instructor/history?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Student Records</a></div></div><div class="grid" style="margin-top:18px"><div class="stat"><span>LIVE CLASSES</span><b>${active}</b></div><div class="stat"><span>TECHNICIANS</span><b>${students}</b></div><div class="stat"><span>CERTIFICATES ISSUED</span><b>${certCount}</b></div><div class="stat green"><span>SYSTEM</span><b>ONLINE</b></div></div><div class="home-grid"><a class="home-card" href="/instructor/build-select?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">📝</div><div class="title">Tests & Hunts</div><div class="desc">Build quizzes and QR scavenger-hunt stations.</div></a><a class="home-card" href="/instructor/simulations?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">🛠️</div><div class="title">Diagnostic Simulations</div><div class="desc">Review technician diagnostic paths, scores, penalties, and final diagnoses.</div></a><a class="home-card" href="/instructor/reports?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">🏆</div><div class="title">Reports & Certificates</div><div class="desc">Print records and certificates for completed technicians.</div></a><a class="home-card" href="/instructor/feedback?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">★</div><div class="title">Training Feedback</div><div class="desc">Review technician ratings and comments.</div></a><a class="home-card" href="/instructor/admin?pin=${encodeURIComponent(INSTRUCTOR_PIN)}"><div class="icon">⚙️</div><div class="title">Admin & Content</div><div class="desc">Courses, wording, content, and training-system settings.</div></a></div><div class="card" style="border-top:5px solid var(--red)"><div class="section-title"><h2>Live & Recent Classes</h2><span class="pill open">v5.2 ACTIVE</span></div><div style="overflow:auto;margin-top:12px"><table><thead><tr><th>Class</th><th>Code</th><th>Students</th><th>Status</th><th>Action</th></tr></thead><tbody>${rows||'<tr><td colspan="5">No classes yet. Click Start New Class above.</td></tr>'}</tbody></table></div></div>`));
});
...
app.get('/instructor/simulations',auth,async(req,res)=>{
  ...
  const assignments=(await pool.query(`SELECT * FROM simulation_assignments`)).rows; const amap=new Map(assignments.map(x=>[`${x.class_id}:${x.simulation_key}`,x]));
  const sims=Object.values(DIAGNOSTIC_SIMULATIONS).map(sim=>{const classSetup=classes.map(c=>{const a=amap.get(`${c.id}:${sim.key}`), selected=a?.root_cause_key||''; return `<div style="border:1px solid #ddd;border-radius:12px;padding:14px;margin:10px 0"><div class="section-title"><div><b>${esc(c.course)}</b><div class="muted">Class ${esc(c.code)} · ${c.student_count} technician(s) · ${c.active?'Active':'Closed'}</div></div>${selected?'<span class="pill open">ROOT CAUSE SET</span>':'<span class="pill">NOT RELEASED</span>'}</div><form method="post" action="/instructor/simulations/${encodeURIComponent(sim.key)}/configure?pin=${encodeURIComponent(INSTRUCTOR_PIN)}" style="margin-top:10px"><input type="hidden" name="class_id" value="${c.id}"><label>Planted Root Cause<select name="root_cause_key" required><option value="">Select the failure BEFORE students begin</option>${sim.rootCauses.map(r=>`<option value="${esc(r.key)}" ${selected===r.key?'selected':''}>${esc(r.diagnosis)}</option>`).join('')}</select></label><button>${selected?'Update Root Cause & Release':'Set Root Cause & Release Simulation'}</button></form>${selected?`<div class="success" style="margin-top:8px"><b>Instructor Only:</b> ${esc(rootCauseByKey(sim,selected)?.diagnosis||'')}</div>`:''}</div>`}).join('')||'<p>No classes exist yet.</p>'; return `<div class="card" style="border-top:5px solid var(--red)"><div class="eyebrow">AVAILABLE SIMULATION</div><div class="big">${esc(sim.title)}</div><p><b>Model:</b> ${esc(sim.model)} &nbsp; · &nbsp; <b>Work Order:</b> ${esc(sim.workOrder)} &nbsp; · &nbsp; <b>Hours:</b> ${esc(sim.hours)}</p><div class="alert"><b>Customer Complaint:</b> ${esc(sim.complaint)}</div><p><b>Instructor setup required:</b> Select the planted root cause for each class. Students cannot enter the diagnostic bay until you release it.</p>${classSetup}<div class="toolbar"><a class="btn light" href="/instructor/simulations/${encodeURIComponent(sim.key)}/preview?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Preview Simulation & Root Causes</a></div></div>`}).join('');
  res.send(layout('Diagnostic Simulations', `<div class="toolbar"><a class="btn light" href="/instructor/dashboard?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Instructor Command Center</a></div><div class="card" style="background:linear-gradient(135deg,#171717,#4b090d);color:#fff;border-bottom:6px solid var(--red)"><div class="eyebrow" style="color:#ffb9bd">Diagnostic Failure Simulation System · v5.2</div><div class="big" style="font-size:30px">Simulation Library</div><p style="margin-bottom:0">The instructor selects the planted root cause before students can start.</p></div>${sims}<div class="card"><div class="big">Technician Attempts</div><p>Permanent record of each technician's diagnostic path, test sequence, parts-cannon penalties, final diagnosis, repair selection, score, and elapsed time.</p><div style="overflow:auto"><table class="sim-history"><thead><tr><th>Technician</th><th>Dealer</th><th>Simulation</th><th>Score</th><th>Penalty</th><th>Date</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="7">No technician has run a simulation yet.</td></tr>'}</tbody></table></div></div>`,simulationCss()));
});
...
app.post('/instructor/simulations/:key/configure',auth,async(req,res)=>{
  ...
  const classId=Number(req.body.class_id), rootKey=(req.body.root_cause_key||'').trim(); if(!classId||!rootCauseByKey(sim,rootKey)) return res.status(400).send('Choose a valid class and root cause.');
  ...
  await pool.query(`INSERT INTO simulation_assignments(class_id,simulation_key,root_cause_key,enabled,configured_at) VALUES($1,$2,$3,true,now()) ON CONFLICT(class_id,simulation_key) DO UPDATE SET root_cause_key=$3,enabled=true,configured_at=now()`,[classId,sim.key,rootKey]);
...
app.get('/instructor/simulation/:attemptId',auth,async(req,res)=>{
  ...
  res.send(layout('Simulation Attempt', `<div class="toolbar"><a class="btn light" href="/instructor/simulations?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">← Simulations</a><a class="btn light" href="/instructor/student/${a.student_id}?pin=${encodeURIComponent(INSTRUCTOR_PIN)}">Student Record</a></div><div class="card"><div class="eyebrow">Permanent Simulation Attempt #${a.id}</div><div class="big">${esc(a.name)} · ${esc(sim.title)}</div><p>${esc(a.dealer)} · Class ${esc(a.code)}</p><div class="grid"><div class="stat"><span>SCORE</span><b>${a.score}%</b></div><div class="stat"><span>EARNED</span><b>${a.positive_points}</b></div><div class="stat"><span>PENALTY</span><b>-${a.penalty_points}</b></div><div class="stat"><span>STATUS</span><b>${esc(a.status)}</b></div></div></div><div class="card"><div class="big">Final Decision</div><p><b>Diagnosis:</b> ${esc(a.final_diagnosis||'Not submitted')} ${a.diagnosis_correct?'✓':'✗'}</p><p><b>Correct Diagnosis:</b> ${esc(a.assigned_diagnosis||sim.correctDiagnosis)}</p><p><b>Repair:</b> ${esc(a.final_repair||'Not submitted')}</p><p><b>Correct Repair:</b> ${esc(a.assigned_repair||sim.correctRepair)}</p></div><div class="card"><div class="big">Diagnostic Path</div>${rows||'<p>No actions recorded.</p>'}</div>`,simulationCss()));
});
